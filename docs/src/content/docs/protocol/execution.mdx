...
| --- | --- | --- |
| Context / input | `context_id`, `account_id`, `device_id`, `input`, `read_register` | Read who is calling — as a person (`account_id`) and as a machine (`device_id`) — in which context, with what arguments. |
...
That artifact is the bridge back to the core: its committed writes are packaged as **data-plane operations** (`Put` / `Delete`) in the context scope — each stamped with the executor's identity, the scope's current heads as parents, and a hybrid timestamp. From there they are exactly the same [operations](/protocol/operations/): appended to the DAG, folded into the [root](/protocol/projection/), and ready to broadcast.

:::note[Handoff]
**Execution ends where the write path begins.**

This chapter stops at "an outcome exists." **The [write path](/protocol/write-path/)** picks it up: how the outcome is signed, persisted atomically, and broadcast to the context's members.
:::
...
| Family | Signature |
| --- | --- |
| Input | `context_id(reg: u64)`, `account_id(reg: u64)`, `device_id(reg: u64)`, `input(reg: u64)`, `xcall_origin(reg: u64) -> u32` |
...
We now know where data operations originate. **[The Write Path](/protocol/write-path/)** follows one out of the runtime: how an outcome becomes a signed, durably persisted, broadcast operation.
