...
| 9 | `CAN_AUTHOR_ON_BEHALF` | `1 << 9` | Execute a [delegated write](/protocol/delegated-authorship/) as another member. The one bit that resolves through the anchor — see the aside below. |
...
<Aside type="caution" title="One bit resolves through the anchor: CAN_AUTHOR_ON_BEHALF">
[Delegated authorship](/protocol/delegated-authorship/) is the single documented exception to the paragraph above. `holds_authorship` — the check behind both `POST /admin-api/contexts/:context_id/intents` and the delegated-delta apply path — resolves in two steps:

1. `effective_capabilities(target)`. A direct grant, or one written for an inherited member at the target subgroup, wins and is the answer.
2. Otherwise, if the path is `Inherited { anchor, .. }`, read `member_capability(anchor)` and honour a `CAN_AUTHOR_ON_BEHALF` found there.

Membership stays the outer bound. Step 2 is only reached once step 1 has already resolved the walk, so a `Restricted` wall and the target subgroup's deny-list refuse exactly as they do for every other bit: **a grant reaches wherever membership reaches, and no further.**

Note the `Inherited` in step 2. A **direct** member of the target does not reach the ancestor grant — `check_path` short-circuits on a direct row and computes no anchor — so a group that admitted the node in its own right decides for itself. The fleet path never lands there: a TEE admission into an `Open` subgroup runs through `admit_member_if_absent`, which gates on the inheritance-aware `is_member` and writes no row for a node that already inherits. A node that *did* need its own admission is exactly the node that needs its own grant.

The exception exists because of the shape a TEE relay fleet actually has — admitted once at the namespace root, executing for contexts that live in subgroups. Under the general rule a namespace-wide grant authorized nothing below it, and there is no row to write for an inherited member short of admitting the node directly to every subgroup. No other bit resolves this way. `CAN_JOIN_OPEN_SUBGROUPS` may look similar, but it is read at the anchor by construction — it is the bit that decides the walk — not by exception to it.
</Aside>
...
That completes the governance picture — who is a member, how membership flows down the tree, and how capabilities resolve. Next, **[Applications, Bundles & Services](/protocol/applications/)**: the code a context actually runs, and how it is packaged and bound.
