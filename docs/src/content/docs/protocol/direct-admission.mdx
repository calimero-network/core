...
3. It publishes that op on the namespace topic, **sealed**: under the key covering
   the invitation's group when it holds one, and otherwise by handing the signed
   op back to the admitter it just talked to, which wraps it as
   `NamespaceOp::RootRelaySealed` and publishes under its own envelope. Either
   way the account, the group and the join time stay off the topic. There is no
   cleartext third option — a join with nowhere to be sealed fails instead of
...
That the unkeyed case can route through the admitter at all is a consequence of
step 2's endorsement requirement rather than a new dependency: a joiner that
reached no admitter has already failed, so the peer it asks to seal is one it has
