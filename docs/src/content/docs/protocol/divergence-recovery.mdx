...
Three lighter signals exist to keep a partitioned namespace from sitting idle once connectivity returns:

- **Readiness beacon** - a peer's beacon carries its `applied_through` and governance `dag_head`. `beacon_indicates_divergence` fires only when the local node already holds state and the beacon's head op is **not present locally** - a confirmed gap that picks a sync partner. The check runs against a fresh DAG read, never at receive time. A beacon that *fails* the namespace-membership check is not simply dropped: if it carries a verifiable admission proof, the receiver still pulls governance, targeted at the beacon's own signer rather than an arbitrary mesh peer, since that peer is the only one holding its own not-yet-gossiped join op. That pull never writes a membership row and never populates the readiness cache; membership itself still arrives only through a verified `MemberJoinedAt` op.
...
- **Membership rebroadcast** - a joiner's signed `MemberJoinedAt` publish that collected zero acknowledgements is queued and rebroadcast **verbatim** (never re-signed) the next time a namespace peer becomes reachable. This closes a case the beacon signal alone cannot: a join whose broadcast reached no existing member leaves every peer with no divergent head to detect in the first place. While the op is queued, the joiner's own outgoing beacons also carry its invitation as an admission proof, so a peer that has not yet seen the membership op can still verify the joiner belongs before that op arrives.
...
- **Debounce on the consumer.** A beacon that indicates divergence fires at most one namespace governance sync per `NS_BEACON_SYNC_DEBOUNCE` (5 s) **per namespace** - without it a behind node would launch one sync per beacon per peer (`crates/node/src/handlers/network_event/readiness.rs:31-74`). The provable-non-member path above funnels through this same beacon-triggered-sync call, targeted at the beacon's signer rather than an arbitrary mesh peer.
- **Probe-response rate-limit on the producer.** A peer answers readiness probes at most once per `beacon_interval / 2` per `(peer, namespace)` pair. This closes both a traffic-amplification path (one small probe otherwise pulls a signed beacon from every Ready peer on the topic) and a mailbox-CPU path; the budget keys on `(peer, namespace)`, so varying the probe nonce can't bypass it (`crates/node/src/readiness.rs:919-945`).
- **Clock-drift rejection.** A beacon whose timestamp is more than `MAX_BEACON_CLOCK_DRIFT_MS` (60 s) ahead of local wall-clock is rejected outright before it reaches the readiness cache (`crates/node/src/readiness.rs:338-347`). The window tolerates legitimate NTP-synced drift while closing a cache-poisoning vector - a forged far-future beacon that would otherwise lodge in the readiness cache and never age out. The provable-non-member path applies the same bound symmetrically (past as well as future), via a standalone check, because a beacon that never reaches the cache has no per-peer entry to compare against (`crates/node/src/handlers/network_event/readiness.rs:76-85`).
