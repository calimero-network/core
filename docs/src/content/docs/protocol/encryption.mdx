...
**A delivery is unverifiable by construction.** The envelope apply takes an `expected_key_id` and rejects a key whose `SHA256` does not match it, which is what makes a responder untrusted for *content* rather than merely less-preferred. A `KeyDelivery` names no `key_id`, so there is nothing to compare against and the DAG path passes `None`. That is not a shortcut — no hash exists — but it does mean the delivered bytes are accepted on the strength of who sent them alone.
