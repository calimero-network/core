...
| Governance **Root** ops, cleartext set (`MemberJoined`, `MemberJoinedAt`, `NamespaceCreated`) | **No** | — | Unsealable rather than deliberately public. The two invitation joins are published by the JOINER, which holds no namespace key and obtains one *because* the op is published — sealing them under that key is circular. `NamespaceCreated` is genesis, before any key exists. The other two joins are published by a principal that already holds the key (an inherited member for `MemberJoinedOpen`, the admitter for `MemberJoinedViaTeeAttestation`), so they seal. |
