...
| Governance **Root** ops, sealed set (`GroupCreated`, `GroupReparented`, `GroupDeleted`, `AdminChanged`, `PolicyUpdated`, `KeyDelivery`) | **Yes** | Namespace key | AES-256-GCM over `borsh(RootOp)`, carried as `NamespaceOp::RootSealed { key_id, encrypted }`. Published by a principal that already holds the namespace key, so nothing is lost by sealing. `root_op_is_sealable` (`crates/governance-types/src/lib.rs`) is the single place the split is decided. |
| Governance **Root** ops, cleartext set (`MemberJoined`, `MemberJoinedAt`, `MemberJoinedOpen`, `MemberJoinedViaTeeAttestation`, `NamespaceCreated`) | **No** | — | Unsealable rather than deliberately public: the four joins are published by a joiner that does not hold the key yet, and `NamespaceCreated` is genesis, before any key exists. |
...
- **Group structure and membership churn** — `group_id` and `key_id` tags on governance ops, plus the cleartext join Root ops (`MemberJoined`, `MemberJoinedAt`, `MemberJoinedOpen`, `MemberJoinedViaTeeAttestation`), expose who joined which group and when. `KeyDelivery` no longer adds to this — it is sealed, so who was handed a key, and at which causal position, stays inside the namespace. Removals are `GroupOp`s and were never cleartext.
