...
| Ephemeral presence slice (`BroadcastMessage::Ephemeral.ciphertext`) | **Yes** | Current key of the keyring covering the context's group (`from_sk`) | AES-256-GCM with a fresh nonce per publish; the envelope is signed by its author. The keyring comes from `key_covering_group`, as for the state delta, so presence is readable by exactly the readers of the context's state. Only that keyring's current key is accepted on receipt, so a rotation is what evicts a departed member. |
...
| Governance **Root** ops, cleartext set (`NamespaceCreated`) | **No** | — | Genesis, published before any key exists. `MemberJoined` / `MemberJoinedAt` still answer `false` to `root_op_is_sealable`, but that is now a statement about the CLEARTEXT form of the variant and not about how a join travels: every join is published sealed, either by a joiner that holds the covering key (`RootSealed` / `RootSealedForGroup`) or by an admitter wrapping the joiner's signed op (`RootRelaySealed`). The arm must stay `false` because the joiner's inner op *is* a cleartext `Root(..)` — the seal is the admitter's envelope around it — and flipping it would make the publish-side guard refuse the joiner's own local apply. |
| A **relayed** invitation join (`NamespaceOp::RootRelaySealed`) | **Yes** | Namespace key | The way an invitation join *can* be sealed. When an admitter relays a join for a keyholder with no node of its own (`POST /admin-api/namespaces/:id/admit`), it seals the joiner's **whole signed op** — `AES-256-GCM(borsh(SignedNamespaceOp))` — and signs the envelope itself. Receivers decrypt, verify the JOINER'S inner signature, and apply, so `join_op_proves_ownership` still decides who joined and a hostile admitter still cannot substitute an account. Nesting the signed op rather than moving the signature inside the seal is what preserves keyless verification: the outer envelope is admitter-signed, so a peer with no namespace key authenticates what it stores exactly as for `RootSealed`. It is also how a joiner *with* a node publishes a join it cannot seal: it holds the covering key on the ordinary path (the join bundle delivers it before the publish) and seals for itself, and when it does not it hands the signed op to the admitter over the namespace-join stream (`InitPayload::RelaySealedJoinRequest`) rather than publishing in the clear. That costs no availability — since [direct admission](/protocol/direct-admission/) a join with no admitter endorsement fails outright, so the first peer asked is one the joiner has already completed an exchange with, and a refusal falls through to the rest of the namespace topic (an admitter awaiting its own key delivery endorses and serves no envelope, which is one of the ways a joiner ends up unkeyed) — and a relay that finds no keyholder **fails the join**, because a fallback to cleartext would make "sealed" and "leaked" the same silence. |
...
A cleartext subgroup-targeted join is **refused at apply**, so sealing is a rule rather than a convention. The decision lives there and not in `root_op_is_sealable` because that function is `const` and sees only the op, which cannot tell whether the invitation's `group_id` is the namespace root.

A namespace-root joiner genuinely holds no key — its key arrives only in answer to the join it is publishing — so it cannot seal for itself, and the cleartext form of that join is still what the apply path accepts. What changed is that nobody publishes it: instead of falling back to a cleartext broadcast, such a joiner hands its signed op to the admitter that endorsed it and the admitter publishes it as `RootRelaySealed` (see the table row above). The cleartext acceptance therefore remains for the inner op a receiver reads *after* decrypting, and for a joiner speaking to a pre-relay peer, which fails the join rather than leaking it.
...
- **Group structure and membership churn** — `group_id` and `key_id` tags on governance ops expose that a namespace gained a member and when. The joins themselves no longer do: a joiner holding the covering key seals its own (`RootSealed` / `RootSealedForGroup`) and one that does not routes it through an admitter (`RootRelaySealed`), so neither form names the account or the group on the wire. The rest no longer adds to this: `KeyDelivery`, `MemberJoinedOpen` and `MemberJoinedViaTeeAttestation` are sealed, so which subgroup an inherited member reached, which fleet replica was admitted with what attestation measurements, and who was handed a key at which causal position all stay inside the namespace. Removals are `GroupOp`s and were never cleartext.
