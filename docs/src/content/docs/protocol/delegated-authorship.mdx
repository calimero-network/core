...
So a node may be run as a relay, with `merod init --delegated-access` (or `server.admin.delegated_access = true` in `config.toml`; `--public-intents` and `public_intents` are accepted as aliases for one release). It opens the two routes above, and lets the guard accept a request-carried proof in place of a token — every route it opens carries its own credential:
