...
**A namespace is created carrying it in its default-capability mask**, so a member of one — an attested fleet relay included — is able to relay with no further op. That is what makes hosted delegated execution work without an admin present at the moment a node is assigned.

The mask is **resolved, not copied**. A member's capability row records an *explicit* grant; where there is none, the group's default answers, read at the point the question is asked. That is what makes the mask independent of ordering: it used to be copied into the row at admission, so whether a relay could author depended on whether `DefaultCapabilitiesSet` had folded before the admission did — and no peer controls that, because the key-delivery retry replays buffered ops per signer rather than in causal order. Two peers could therefore materialise different rows for the same relay and disagree permanently about whether its writes were authorized. Resolving removes the ordering from the answer instead of arranging it.
...
- **It follows the mask, and never reaches admins.** Because the default is resolved rather than snapshotted, a member with no explicit grant tracks the group's current mask — including one admitted before it was ever set. Changing the mask therefore moves every such member, which is what "default" means here; a member who must not move needs an explicit grant of their own. Admins are outside this entirely: the capability is not implied by admin, and an admin who needs it is granted it directly.
