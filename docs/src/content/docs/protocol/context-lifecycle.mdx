...
- **`application_id`** and **`bytecode_id`** — the real target application and its pinned bytecode id. Carrying these lets the joiner **align on the same app schema** the inviter is on. Without them the joiner would seed its group metadata to zero and diverge from the inviter's state-hash view permanently.
...
<SeqDiagram id="ctx-invite-join" title="Invite → join handshake"
  ...
  steps={[
    {from:0,to:0,label:'create_group_invitation → SignedGroupOpenInvitation (signed, expiring; carries group_id, app_id, bytecode_id)'},
