...
Both forms exist permanently. A device link travels as an *encrypted* group op, so a node must already hold the scope key to enroll a device — if the only way to receive a key were device-addressed, a new member could never obtain the first one. Member addressing is therefore the bootstrap path, not a legacy one.
...
- The requester's member key has **no account** in the scope → wrap to its identity. This is the bootstrap case and cannot be retired: enrolling a device needs an encrypted group op, hence the key, so a keyless joiner has no device the scope has heard of.
