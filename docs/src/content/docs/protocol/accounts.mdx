...
An `AccountId` is *derived* — from the node's account root and the namespace — so
it appears on no wire and in no operation. That makes it recoverable from the root
alone, and it also means a node has to be asked:

```bash
meroctl account show <NAMESPACE_ID>     # GET /admin-api/namespaces/:id/account
```

It answers before `account create` has ever run: the id exists as soon as the node
has a root. What is absent until then is the **device** — an account nobody has
heard of yet, with nothing yet speaking for it.
