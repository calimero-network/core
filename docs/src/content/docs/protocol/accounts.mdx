...
A member and the device it joined with become known **together**, in one operation, because the three cleartext join operations (`MemberJoined`, `MemberJoinedAt`, `MemberJoinedOpen`) carry the joiner's credential as a required field:
