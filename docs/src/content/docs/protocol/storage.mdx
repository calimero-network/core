...
struct ContextDagDelta { // Delta column (legacy data DAG)
  ...
  checkpoint_root_hash: Option<[u8; 32]>, // snapshot boundary only
...
| `0x36` | NamespaceParticipation | `0x36 ‖ namespace_id(32)` → marker (no key material) |
...
| `ContextActivatedBlob` | per-context | The bytecode blob this context **last activated** (`activated_bytecode()`) — set when a migration commits or a code-only swap applies, and moved forward only. The single up-to-date check everywhere is `marker == group.bytecode_id`; if it lags the group's current bytecode id, this node has not caught up to the activated code. |
