...
| --- | --- |
| Creating a context whose application row carries no coordinates | `create_context` fails: `application <id> has no registry coordinates to register a context with` |
| Emitting an upgrade for such a row | the upgrade is refused: `application <pkg>@<ver> has no registry coordinates to upgrade with` |
