...
and a signed bundle's **`ContentHash`** (a digest of a file's raw bytes) is only
ever supplied by a human, e.g. a CLI flag. `BlobId` and `ContentHash` are distinct
types with no implicit conversion between them: assigning one to the other does
not compile. Converting requires going through raw bytes explicitly, so a swap
has to be written on purpose rather than happening by accident.
