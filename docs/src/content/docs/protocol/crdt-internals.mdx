...
<SeqDiagram
  ...
  steps={[
    ...
    { from: 0, to: 1, label: 'gossip delta (alice = vA)', color: '#a5ff11' },
    { from: 1, to: 0, label: 'gossip delta (alice = vB)', color: '#a5ff11' },
    { from: 1, to: 1, label: 'lww_pick: equal HLC ⇒ SHA-256(value) tiebreak', color: '#a371f7' },
    { from: 0, to: 0, label: 'lww_pick: same compare ⇒ same winner', color: '#a371f7' },
...
if env::in_merge_mode() {
    ...
    Self { value, timestamp: env::hlc_timestamp(), node_id: env::device_id() }
}
```
...
That is precisely what makes identity-gated migration possible. An owner re-stamps its identity-gated entries at a new version on its next ordinary signed write (`with_schema_version`, `crates/storage/src/entities.rs:689-705`) without diverging peers' hashes and without a coordinated breaking schema upgrade. A `None` tag reads as a legacy/v0 entry.
