...
| `POST /tee/attest` | `{ nonce, application_id?, bindNodeKey?, bindTransportKey? }` | Generates a TDX quote over `nonce \|\| app-hash` (or the key / transport bindings above); rejects a mock result unless `--mock-tee`. |
