...
3. **Any existing member verifies and admits.** A current member receives the
   ...
   account }`. No special "verifier" role exists — any member may admit a valid quote.
