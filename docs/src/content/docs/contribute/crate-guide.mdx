...
| `auth` (`mero-auth`) | operator | Forward-auth service: JWT issue / verify, pluggable providers. | Runs embedded in the server or behind a reverse proxy. `user_password` is the only shipped provider. |
