...
Internally a map keyed by `AccountId`. `insert` always writes the **current
...
impl KvStore {
    ...
    // read another user's slot by their account
    pub fn get_user_for(&self, user_key: AccountId) -> app::Result<Option<String>> {
...
account of whoever pushed it. Anyone may `push`, but `update`/`tombstone` on
...
impl TeamDoc {
    #[app::init]
    pub fn init() -> TeamDoc {
        let owner: AccountId = calimero_sdk::env::account_id().into();
    ...
    pub fn add_owner(&mut self, who: Vec<AccountId>) -> app::Result<()> {
