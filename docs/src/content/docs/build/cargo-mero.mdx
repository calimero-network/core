...
```bash
# From crates.io (published with each core release, like mero-sign):
...
| ------- | ------------ |
| `cargo mero new <name>` | Scaffold a new app (Cargo.toml, build.rs, lib.rs with a TestHost test). |
| `cargo mero build` | Compile to wasm32, optimize, and embed the ABI into `res/<name>.wasm`. |
...
1. **Scaffold.**
   `cargo mero new my-app` writes a crate with `Cargo.toml` (SDK pins, the `[package.metadata.calimero]` app id, and the `app-release` / `app-profiling` profiles), `build.rs`, `src/lib.rs` (state, events, logic, and a `#[cfg(test)]` TestHost test), and `tests/converge.rs`.
