...
## Example: a high-water-mark gauge (`merge = max`)

Delegation (one `field.merge()` per field) is the common case, but a hand-written
...
/// A monotonic high-water mark: the merged value is the larger of the two.
...
impl Mergeable for HighWaterMark {
    fn merge(&mut self, other: &Self) -> Result<(), MergeError> {
...
The determinism rule: a `merge` must be **commutative, associative, and
...
<SeqDiagram
  id="d-adv-root-merge"
  title="Root-state merge routes through WASM during sync"
  lanes={[
    { label: 'Peer', color: '#58a6ff' },
    { label: 'Node (host)', color: '#3fb950' },
    { label: 'WASM app', color: '#a371f7' },
  ]}
  steps={[
    { from: 0, to: 1, label: 'sync: root-entity delta (peer bytes)' },
    { from: 1, to: 1, label: 'host cannot deserialize the app state type', color: '#3fb950' },
    { from: 1, to: 2, label: '__calimero_merge_root_state(local, incoming)', color: '#a371f7' },
    { from: 2, to: 2, label: 'deserialize as T; run Mergeable::merge', color: '#a371f7' },
    { from: 2, to: 1, label: 'merged root bytes', dashed: true, color: '#a371f7' },
    { from: 1, to: 1, label: 'commit; re-check root hash', color: '#3fb950' },
    { type: 'divider', label: 'typed merge ⇒ converged root' },
  ]}
/>
...
`ReplicatedGrowableArray`, `FugueText`, `RichText`, `RichDocument`), access-control collections (`SharedStorage`,
...
:::caution[Type aliases defeat the private-storage rewrite]
To make tree-backed collection fields node-local, `#[app::private]` injects
`PrivateStorage` as the trailing generic on `UnorderedMap`, `UnorderedSet`,
`Vector` (and the sorted variants). It matches on the **last path segment** of
the type, so a fully-qualified path (`calimero_storage::collections::UnorderedMap`)
is covered — but a type alias or `use … as` rename is **not**. An aliased
collection silently keeps its default `MainStorage` and its entries leak into the
synced tree, defeating the whole point of `#[app::private]`. Write the collection
type out by name (or pass the explicit `PrivateStorage` parameter yourself);
never reach a private field through an alias.
:::
...
unchanged (`counter: old.counter`) or use `increment_for(device_id)` with a
deterministic id.
:::
...
The app's state schema (the wasm-ABI manifest) is embedded as a wasm **custom
...
- **Reads are validation-gated and fail-open.** Loading the embedded schema
  returns `None` if the section is absent *or* malformed *or* well-formed JSON
  that doesn't pass manifest validation (dangling refs, bad state root). A
  missing schema doesn't brick a module — pre-schema modules simply get the
  default (write-lock, no downgrade check) behaviour.
- **Writes fail-closed.** Embedding the schema (build-time tooling) refuses a
  malformed or truncated module rather than emitting a corrupt one.

### The identity-downgrade rail

Calimero's upgrade gate (and the `cargo mero abi diff` CI lint) compare the old and
...
:::caution[The downgrade check fails closed on unresolvable types]
When the gate resolves a field's top-level type, an **unresolvable** type — a
reference cycle, a dangling `$ref`, or an unknown `crdt_type` — is *not* graded
as "plain". Folding it into "plain" would let an identity-gated field hidden
behind a broken ref slip past the gate as a missed downgrade. So an old field
that's gated-or-unknown is always compared against its new shape, and an
unresolvable new type reads as a downgrade. The only carve-out is when *both*
old and new are unresolvable and unchanged — a legacy app that has always had a
dangling ref and never used gated state.
:::

## Multi-service bundles

A bundle is usually one WASM module — the manifest's top-level `wasm` (+ optional
...
```text
meroctl context create <app_id> --service <name>
```

At resolve time a `None` selection is only unambiguous for a single-service app or a
...
|---------|--------------|
| `account_id` vs `device_id` vs `context_id` vs `xcall_origin` | `env::account_id()` is *who* is calling (a person; stable across their devices). `env::device_id()` is *which machine* (one installation; the replica writes are attributed to). `env::context_id()` is the current context. `env::xcall_origin()` is the node-set, unforgeable origin of an xcall (`None` for a direct call). |
...
<CardGrid>
  <LinkCard title="CRDT internals" href="/protocol/crdt-internals/" description="The full merge-dispatch path and id derivation behind the SDK." />
  <LinkCard title="Cross-context calls" href="/protocol/xcall/" description="The dispatch and namespace model behind env::xcall." />
  <LinkCard title="Gotchas & sharp edges" href="/build/gotchas/" description="The non-obvious facts that bite once a replica diverges." />
</CardGrid>
