...
| `env::device_id() -> [u8; 32]` | The 32-byte public key of the **device** executing this call — one installation. | Differs between two devices of the same person, which makes it right for per-replica state and wrong for per-user state. Storage collections that stamp an owner (`UserStorage`, `AuthoredMap`, `SharedStorage`) key off this, so a lookup into one must use it too. |
