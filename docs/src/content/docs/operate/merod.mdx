...
`revoke-proof` and `link-proof` reach the account root through `--from <PHRASE>`.
...
merod --node node1 account <export|import|device|sign-cert|revoke-proof|link-proof>

# Offline: no --node, no --home, no init.
merod account <warrant|login-statement>
merod account <sign-cert|revoke-proof|link-proof> --from ./phrase.txt
```
