...
Context and application aliases are unique across the whole node; an identity
alias only has meaning **within the context it was created in**, so the same
name can resolve to different keys in different contexts.
...
<Aside type="caution">
  ...
  the context/application/identity aliases above.
</Aside>
