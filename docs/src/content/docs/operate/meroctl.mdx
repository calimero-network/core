...
Context and application aliases are unique across the whole node.
...
<Aside type="caution">
  ...
  the context/application aliases above.
</Aside>
...
| `create`                           | Create a new context.                             | `--application-id`/`-a`, `--group-id` (required), `--params`/`-p`, `--watch`/`-w` `<path>`, `--seed`/`-s`, `--name` (context alias), `--service`, `--identity-secret`, `--group-name`, positional `metadata` (requires `--watch`) |
