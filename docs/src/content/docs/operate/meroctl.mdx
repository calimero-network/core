...
| `install`                          | Install an application.                           | `--path`/`-p`, `--url`/`-u`, `--metadata`/`-m`, `--content-hash` (alias: `--hash`), `--watch`/`-w`, `--package`, `--version` |
...
| `update`                           | Update the application in a context.              | `--context`/`-c`, `--application-id`/`-a`, `--path`, `--metadata`, `--watch`/`-w` |
...
A complete run from an installed WASM module to reading back state. Each command
