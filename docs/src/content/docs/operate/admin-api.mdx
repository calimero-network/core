...
<SeqDiagram
  ...
  steps={[
    ...
    { from: 2, to: 2, label: 'run app method → state delta', color: '#a5ff11' },
...
| Method | Path | Purpose |
| --- | --- | --- |
| POST | `/admin-api/install-application` | Install `package`@`version` from the node's configured registry. |
...
| POST | `/admin-api/groups/:group_id/members` | Add group members, each named by account, or by key written `key:<64-hex>`. Every id is hex, so the tag is what tells the two apart — an untagged key is read as an account. |
| POST | `/admin-api/groups/:group_id/members/remove` | Remove group members. |
| POST | `/admin-api/groups/:group_id/accounts/:account/seal` | Seal a payload to that account's **root key**, as this group sees it. Body `{ plaintext }`, hex, at most 64 KiB and never empty; returns `{ accountRootEpoch, ephemeralPublicKey, nonce, ciphertext }`. Caller and target must both be group members, and a non-member is refused identically to an unknown account. Confidentiality only — the sender key is ephemeral and unauthenticated, so anyone knowing a root public key can produce an envelope for it. |
...
| PUT | `/admin-api/groups/:group_id/settings/tee-admission-policy` | Set the group's TEE admission policy: measurement lists, or `signedRelease` to admit any signed mero-tee release of the named profiles. |
...
| GET | `/admin-api/blobs/:blob_id` | Download a blob. Optional `?context_id=<id>`: if this node does not hold the blob, look for it among the context's peers and download it from one. |
| HEAD | `/admin-api/blobs/:blob_id` | Get blob info/metadata (headers only). Optional `?context_id=<id>`: see below. |
| DELETE | `/admin-api/blobs/:blob_id` | Delete a blob. |

### `HEAD` with `?context_id=`

Without the parameter, `HEAD` answers only about blobs this node holds — one
local store read, and a 404 for anything else.

With it, a local miss falls through to the same bounded peer search `GET` uses,
but stops at the header a holder sends before its first chunk: **`HEAD` never
transfers the blob**. Discovery is opt-in for a reason — a search can run to the
node's 30s discovery deadline, while `HEAD` otherwise reads as a cheap call.

`X-Blob-Source` says where the answer came from, and which other headers you get:

| `X-Blob-Source` | Meaning | Headers |
| --- | --- | --- |
| `local` | This node holds the blob. | `Content-Length`, `Content-Type`, `ETag`, `X-Blob-ID`, `X-Blob-Hash`, `X-Blob-MIME-Type` |
| `peer` | Only a context peer holds it. | `Content-Length` (the size that peer reported, omitted if it reported none), `X-Blob-ID` |

`ETag`, `Content-Type`, `X-Blob-Hash` and `X-Blob-MIME-Type` are absent from a
`peer` answer because every one of them is computed from the bytes — the content
hash from local metadata, the MIME type sniffed from the first chunk — and this
node does not have them. They are omitted rather than guessed. A blob held
neither locally nor by any probed peer is still a 404.

Private blobs need the same signed membership proof on this path as on `GET`; a
peer answers "not held" to an unauthorized probe, so discovery is no weaker and
no stronger than the download path.
...
| POST | `/admin-api/alias/create/device` | Create an alias for a device. |
...
| POST | `/admin-api/alias/lookup/device/:name` | Resolve a device alias. |
