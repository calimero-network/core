...
Where this node gets application bundles from - and there is exactly **one** such place.
The node has no second route behind it, so this section decides both what it can install and
whether it serves application bytecode to peers at all.

| Key | Type | Default | Meaning |
| --- | --- | --- | --- |
| `mode` | `"http"` \| `"dht"` | `"http"` | The node's one application source. `http` fetches from `base_url`; `dht` fetches from context members over blob share. |
| `base_url` | URL | written by `merod init` as `"https://apps.calimero.network/"` | Base of the registry `http` mode resolves versions against. Override at init with `merod init --registry-url <URL>`. Unused in `dht` mode. |
...
| | `http` | `dht` |
| --- | --- | --- |
| Fetches applications from | `base_url`, by `package`/`version` | context members, authorized by membership |
| Serves application bytecode to peers | **no** - neither announced nor served | yes |
| Installs `package@version` by name | yes | no - governance or a local `.mpk` path only |
...
Two variables override the file, applied when `merod run` loads the config:

| Variable | Overrides |
| --- | --- |
| `CALIMERO_REGISTRY_MODE` | `mode` (`http` or `dht`) |
| `CALIMERO_REGISTRY_URL` | `base_url` |

A malformed value is a startup error, not a silent fall back to the file's setting: a
misconfigured node must not start.

### How a version is resolved

The node appends the coordinates carried by the governance op to `base_url` -
...
See [how bytecode reaches a node](/protocol/applications/#how-bytecode-reaches-a-node) for
the full picture.

### When it is misconfigured

There is no fallback, so a misconfiguration surfaces rather than degrading:

| Situation | What you see |
| --- | --- |
| `mode = "http"`, no `base_url`, installing by coordinates | the install fails with `[registry] mode = "http" needs a base_url to fetch from` |
| `mode = "http"`, no `base_url`, a group names bytecode | `no application source is configured` in the log; the context keeps its current version and retries on next access |
| `base_url` points somewhere with nothing published there | the install reports the coordinates as unpublished (retryable - the version simply is not there yet) |
| `base_url` points at a registry that repacks bundles | every fetch fails a `blob id mismatch` and no node moves version |

<Aside type="caution">
The registry must serve the `.mpk` byte for byte.
`bytecode_id` covers the whole envelope, so a mirror that re-gzips the tar, reorders its
entries, or restamps mtimes produces a different blob id: every fetch then fails the check,
and every node stays on the version it already runs.
</Aside>
...
KMS and refuses to start without it. **Create a TEE node with `merod init --kms-url
<URL>`** rather than adding this section afterwards. `init` is what writes the node's
...
fetches the key before anything is written and saves `[tee.kms.phala]` with that URL.
...
unverified KMS would be readable by whoever runs that endpoint.
...
| `allowed_compose_hashes` | array of string (hex) | `[]` | Released KMS compose hashes (32-byte SHA-256, the payload of the KMS's RTMR3 `compose-hash` event). When set, merod replays the KMS's RTMR3 event log against its quote and refuses any other compose file. Unset skips the check with a warning; the signed release policy always sets it. |
| `backend` | `"dstack"` \| `"tdx"` | `"dstack"` | What kind of KMS this is. `tdx` is mero-kms as a GCP TDX cluster replica with no dstack: its RTMR3 is its image's boot measurement, so the five register allowlists pin it and `allowed_compose_hashes` must be empty. An external policy file sets it with `kms.backend`. |
...
<CardGrid>
  <LinkCard title="Networking & discovery" href="/operate/networking/" description="What the [swarm], [bootstrap], and [discovery] keys actually do." />
  <LinkCard title="Hardening & security" href="/operate/security/" description="Which settings keep the admin API and keys safe in production." />
  <LinkCard title="Production deployment" href="/operate/deployment/" description="Put this config behind a reverse proxy and run it as a service." />
</CardGrid>
