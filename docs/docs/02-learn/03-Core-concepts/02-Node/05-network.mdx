...
This document provides an overview of the networking component of our P2P project, which is implemented using the `libp2p` library. The network consists of two types of peers: standard nodes and boot nodes, each serving distinct roles and utilizing specific protocols to facilitate peer-to-peer communication.
...
- **Behavior:**
  - **Configuration:** A standard node can be configured to use zero or more relay servers.
  - **External Address:**
    - **Direct Public External Address:** Nodes with a direct public external address do not require relay servers. These nodes publish their public external address to the Kademlia DHT, making them directly accessible to other peers.
    - **Relayed External Address:** Nodes that do not have a direct public external address, typically those behind a NAT or firewall, can obtain a relayed external address by requesting a reservation at a relay server. Once the reservation is accepted, the node publishes its new external address to the Kademlia DHT. This address includes the relay information, allowing other nodes to connect via the relay server. The relay server is used only for the coordination of the hole punching between two nodes, not for relaying the entire traffic.
  - **Discovery Protocols:**
    - **mDNS:** Used for local network peer discovery, mDNS enables nodes to find peers within the same local network without the need for a central server.
    - **Kademlia:** Used for global network peer discovery and routing, Kademlia facilitates the discovery of peers across the entire P2P network by maintaining a distributed hash table (DHT) of peer addresses.
  - **Connection Management:** A peer, identified via PeerId, can be discovered either via mDNS or Kademlia. mDNS discovery provides local network addresses, while Kademlia discovery provides external addresses. The node maintains information about its connections to peers, including the discovery source. For a discovered external address, the node will only attempt to dial the peer if the same peer is not already connected via a discovered local address. This ensures that local connections are the priority and that there are no unnecessary hole punching attempts.
  - **Message Relaying:** The node participates in the `gossipsub` protocol, relaying messages to all connected peers that support `gossipsub`. This enables efficient and scalable message dissemination across the network.
...
- DNS provides a mechanism for resolving domain names to IP addresses within the P2P network. It utilizes DNS-based service discovery to enable nodes to find peers without relying on a central server.
- **Reference:** [libp2p DNS Documentation](https://github.com/libp2p/specs/blob/master/discovery/DNS.md)
...
#### Kademlia (Kad)
- Kademlia is a distributed hash table (DHT) protocol used for peer discovery and data routing. It uses a XOR metric to ensure efficient and scalable peer lookup. Each node maintains a routing table with information about other nodes, facilitating quick lookups and robust network operation.
