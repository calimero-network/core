...
We have build and deployed demo app so you can try it out imediatelly.

Navigate to https://calimero-network.github.io/only-peers-client/ to access app frontend.

You will be asked to setup app by adding node url. It is the same url you used while starting the node in [Getting-started](./01-getting-started.mdx) `http://localhost:NODE_PORT`
