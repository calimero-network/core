...
| | |
|---|---|
| **Status** | Roadmap — issue-ready. Phase 1 + Phase 2 unifier delivered. |
| **Date** | 2026-05-11 |
...
| **Related** | [#2233](https://github.com/calimero-network/core/issues/2233), [#2237](https://github.com/calimero-network/core/issues/2237), [#2280](https://github.com/calimero-network/core/pull/2280), [#2284](https://github.com/calimero-network/core/pull/2284), [#2298](https://github.com/calimero-network/core/pull/2298) (B1+B2+B3+S13), [#2321](https://github.com/calimero-network/core/pull/2321) (observability), [#2324](https://github.com/calimero-network/core/pull/2324) (B3 admin carve-out), [`docs/adr/0001-shared-storage-concurrent-rotation.md`](../adr/0001-shared-storage-concurrent-rotation.md) |
...
- **B3 admin/creator gap — `MembershipStatus::NeverMember` for the namespace creator** ([#2324](https://github.com/calimero-network/core/pull/2324)) — both branches of `membership_status_at` resolve membership via the `GroupMember` row set. The namespace creator never emits a self-`MemberJoined` op at genesis (their admin status is recorded in `GroupMeta::admin_identity`), so the fast path (`get_group_member_role` returns `None`) and the prefix walk (no `RootOp::MemberJoined` for the creator) both returned `NeverMember`. Result: every state delta authored by the creator was hard-rejected on receivers whose governance heads hadn't yet caught up to the creator's at write time — surfaced as a ~30% flaky `group-3node` e2e. **Fix**: short-circuit at the top of `membership_status_at`: if `is_group_admin(store, group_id, signer)` returns true, return `Member(Admin)` immediately. Mirrors the existing carve-out in `namespace_member_pubkeys`. Limitation: `is_group_admin` reads the *current* meta, so an admin who transferred ownership and signed at an earlier cut is still recognised as admin (over-permissive). Acceptable today; a historical-admin DAG walk is the proper long-term fix and is left for a future RFC pass.

- **HashComparison recovery path blocked by `Shared → Public` action rejection** (surfaced during the `group-3node` log analysis; **deferred**, not in #2324) — even after the B3 admin carve-out lands, the heartbeat-divergence safety net is still broken. When a node detects root-hash divergence with peers and triggers HashComparison sync, every retry errors with `Action not allowed: Cannot change StorageType (e.g., User->Public/User->Frozen/etc)` in `verify_shared_action_upsert` (`crates/storage/src/interface.rs:1844`). Debug log: `existing=Shared { writers: {...}, signature_data: None }` vs `new=Public`. The HashComparison/CRDT merge is generating `Public`-typed actions for entities that the writer already wrote as `Shared`. This is a recovery-path correctness bug separate from B3 — once a primary failure mode is masked by the admin carve-out, the next sync regression won't have HashComparison as a safety net to fall back on. **Tracked as a future PR** against `crates/node/src/sync/hash_comparison*`.

- **C4 forward-only is implicit in BFS-from-target-heads, not retrofitted** ([#2328](https://github.com/calimero-network/core/pull/2328)) — when auditing the three call sites of `membership_status_at` for the C4 lock-in, the property turned out not to need any new enforcement code. The invariant is a structural consequence of `prefix_walk_membership` doing BFS *from* `position.governance_dag_heads` backwards: a `MemberRemoved` op causally after the position is unreachable from those heads and therefore never visited. C4 became a doc + test pass rather than a behavior change. The load-bearing doc block on `prefix_walk_membership` now flags this explicitly so a future refactor that broadens the walk frontier (e.g., to "look at the full transition sequence") trips against the property tests and the canary instead of silently reintroducing the taint-cascade surface.

- **Integration-test claims must match what the test actually exercises** ([#2328](https://github.com/calimero-network/core/pull/2328)) — PR review caught that the C4 integration tests originally named `forward_only_*` all constructed `GovernancePosition` with empty `governance_dag_heads`, hitting Branch 1 (heads-equal fast path) and never invoking the BFS in Branch 3. The forward-only invariant lives in Branch 3; the tests as written only validated the materialized-set conflation. Renamed to `fast_path_*` with a section header that says explicitly what's covered (Branch 1) and what's deferred (Branch 3 BFS via real DAG). Same correction applied to the property test: renamed `prefix_walk_forward_only_property_random` → `resolver_state_machine_add_ending_prefix_is_member` since it tests the resolver mapping in isolation, not the BFS truncation. **Lesson for future Phase 2/3 PRs**: rename or constrain test claims when the test is honestly a unit on one half of a two-stage pipeline.

- **Internal roadmap codes are confusing to outside readers** ([#2328](https://github.com/calimero-network/core/pull/2328)) — `B1` / `B2` / `B3` / `S13` / `C4` / `D1` etc. appear throughout the codebase as a shorthand for "see this RFC item". The shorthand is unreadable without the RFC open. PR #2328 stripped 67 references across 8 files and replaced them with descriptive names ("cross-DAG check", "governance-pending drain", "apply-time membership check", "forward-only invariant", "deny-list"). The mechanical substitution dropped trailing spaces on log prefixes (`"B2:"` → `"governance-pending drain:"` losing the space-after-colon); caught and fixed in PR review across 26 sites. **Going forward**: descriptive names in code, RFC codes only in commit messages / PR descriptions / this doc.

- **D1 keying granularity: per-group, not per-peer-id** ([#2329](https://github.com/calimero-network/core/pull/2329)) — the design discussion considered libp2p connection-gating (drop a peer's traffic at the transport) but rejected it because the same identity can be a member of multiple groups; transport-level gating would drop legitimate traffic for groups the member still belongs to. Each context has its own gossip topic, so a per-`(group_id, identity_pubkey)` deny set is the right granularity. Side benefit: libp2p peer-id rotation has no effect on the deny check, so the "rotate peer-id to bypass" attack is ruled out by construction without needing a dedicated e2e test.

- **D1 drain-path asymmetry is by design, not a gap** ([#2329](https://github.com/calimero-network/core/pull/2329)) — review flagged that `drain_governance_pending` calls `apply_authorized_state_delta` directly and bypasses the entry-point deny-list filter. A buffered delta's `governance_position` predates the eventual removal; the cross-DAG check (with C4 forward-only) correctly resolves it to `Member` and applies. The deny-list is an entry-point optimization for *new* post-removal traffic; the drain path's authorization is decided authoritatively by the cross-DAG check against the buffered delta's original position. Documented inline at the deny-list call site so the asymmetry doesn't keep getting re-flagged.

- **Store API has no transactional batching — D1 atomicity is bounded, not guaranteed** ([#2329](https://github.com/calimero-network/core/pull/2329)) — `apply_group_op_mutations` performs `remove_group_member` then `mark_denied` (and `add_group_member` then `clear_denied` on the inverse path). The store's `handle` is a thin wrapper around the database; each `.put`/`.delete` hits storage immediately and there is no batch-or-commit primitive in the current API. A crash between the two writes leaves the deny-list and the materialized set inconsistent: `Removed → not-yet-denied` is safe (cross-DAG check still rejects); `Added → still-denied` requires another `MemberAdded` apply to clear and falls back to cross-DAG until then. Multi-write atomicity is a store-layer improvement, not addressable inside the deny-list module — documented as a known limitation and left for a future store-API pass.

- **Prefix-collision guard test had two missing entries** ([#2329](https://github.com/calimero-network/core/pull/2329)) — the `distinct_prefixes` test in `crates/store/src/key/group/mod.rs` exists to catch prefix collisions inside `Column::Group`. `GROUP_DENIED_MEMBER_PREFIX` (0x3B) wasn't added when introduced; `GROUP_KEY_PREFIX` (0x3A) had been missing since it was added too. Both added together — when one is missing, the safety net is incomplete; adding only the new entry would invite the same complaint when the next reviewer notices the older gap. **Process note**: any new `Column::Group` prefix must land with its `distinct_prefixes` entry in the same commit; ideally enforce via a `#[test]` that derives the expected set automatically (left for a future S-track item if it becomes load-bearing).

- **Observability ecosystem brought to parity with the rest of the unifier** ([#2320](https://github.com/calimero-network/core/pull/2320), [#2321](https://github.com/calimero-network/core/pull/2321), [infra#81](https://github.com/calimero-network/infrastructure/pull/81)+[#82](https://github.com/calimero-network/infrastructure/pull/82)) — three findings that together closed the loop from "deltas flow correctly" to "we can *see* deltas flowing correctly":
    - **vmagent scraping the wrong ports for the entire history of fuzzy-load-test** — the CI workflow passed `base_port=3001, port_increment=2` to `run-vmagent.sh`, but merobox's `bootstrap run` allocates host RPC ports starting at `DEFAULT_RPC_PORT = 2528` with `base + i`. Every scrape returned `connection refused`; VictoriaMetrics only ever recorded the synthetic `up` series. Fixed by updating the four GHA invocations to `"2528" "1"` and clarifying the script's default args + inline docs.
    - **Sync-protocol metrics defined but never registered or recorded** — PR #2007 added `SyncMetricsCollector` + `PrometheusSyncMetrics` (16 `sync_*` families) but never called `PrometheusSyncMetrics::new(&mut registry)` and never plumbed a collector into `SyncManager`. The trait is now wired (with a static `NoOpMetrics` fallback so test paths don't carry an `Option<_>` branch), and `initiate_sync` records start / complete / failure on every sync attempt.
    - **Node-level state was completely invisible** — added a centralised `crates/node/src/node_metrics.rs` module covering blob cache size + per-reason eviction counters, DAG apply outcomes + cascade-size histogram + missing-parent request counter, B2 governance-pending drain outcomes, NodeState DashMap-size gauges (delta_stores, sync_sessions, governance_pending_contexts/queue_depth, specialized_node_pending_invites), and process resource gauges (RSS, VMS, threads, FDs from `/proc/self`). Recording uses a process-global `OnceLock<NodeMetrics>` so synchronous hot paths (blob-cache eviction, delta-store apply, governance drain) record without signature plumbing. A 30s tick publishes the gauge snapshot. The `merod_build_info{version, peer_id}=1` beacon was added at the same time so operators can verify the metrics pipeline end-to-end (a node that scrapes only `up` + `merod_build_info` is reachable; missing `merod_build_info` means the body never landed).
    - **Grafana dashboard restructured for per-test-run multi-node viewing** — variables now form a hierarchy (`$test_name → $workflow_run → multi-select $merod_name → multi-select $node_name`); every panel filters via regex match + cascades through the hierarchy; all `sum(rate(X))` aggregations preserve `node_name` in their `by` clause so per-node lines render distinctly; every `legendFormat` is prefixed with `{{node_name}}`. The pie-chart panel is the documented exception (aggregates by method on purpose). The same dashboard now lets an operator pick `test_name=scaffolding-e2e` and see all four nodes side-by-side without flipping between instances.
...
**As-shipped notes**:
* `handle_state_delta` calls `membership_status_at(author_id, governance_position)` and matches on the four-variant result: `Member` → continue to apply path; `Removed { last_role }` / `NeverMember` → reject + warn (D1 deny-list now provides the early-rejection path for `Removed` — see [#2329](https://github.com/calimero-network/core/pull/2329)); `Unknown { needed }` → buffer (B2); `Err` (hash mismatch / corruption) → reject + warn.
...
* **Admin/creator carve-out** ([#2324](https://github.com/calimero-network/core/pull/2324)) — `membership_status_at` short-circuits at the top: if `is_group_admin(store, group_id, signer)` returns true, return `Member(Admin)` immediately. Required because the namespace creator does not emit a self-`MemberJoined` op at genesis (their membership lives in `GroupMeta::admin_identity`); without this both the fast path (no `GroupMember` row) and the prefix walk (no `MemberJoined` op) returned `NeverMember` and rejected every state delta from the creator. Mirrors the existing `namespace_member_pubkeys` carve-out. Side-effect of using `is_group_admin`: it reads the *current* meta, so an admin who later transferred ownership is still recognised as admin at any historical cut (over-permissive). Acceptable today; a historical-admin DAG walk is the proper long-term fix. See "Side findings" above for the full incident.
...
#### C4 — Forward-only semantic: enforce as core invariant ✅ DONE

**Phase**: 2 · **Size**: M · **Depends on**: B3, C1, C2 · **Blocks**: — · **Status**: landed in [#2328](https://github.com/calimero-network/core/pull/2328).

**As-shipped notes**:
* Audited the three call sites of `membership_status_at` (`handle_state_delta`, `apply_authorized_state_delta`, `replay_buffered_delta`); all match identically on the four variants and apply forward-only consistently. No path retroactively invalidates pre-cut writes.
* Forward-only is now a **load-bearing doc block** on `prefix_walk_membership` (`crates/context/src/group_store/membership_status.rs`) with an explicit warning that broadening the BFS frontier past `target_heads` reintroduces the taint-cascade surface. The BFS-from-target-heads property is what makes forward-only correct: a `MemberRemoved` op causally after the position's heads is never visited.
* 5 unit tests on the resolver state machine (`prefix_walk_forward_only_*`) covering pre-removal, post-removal, re-add (Add→Remove→Add asserted at three prefix boundaries), role-change, randomized property test on Add-ending prefixes, and a canary that pins the prefix-sensitivity contract (the resolver returns different statuses for `[Added]` vs `[Added, Removed]`).
* 4 integration tests against an in-memory `Store` (`fast_path_*` in `crates/context/src/group_store/tests.rs`). These exercise Branch 1 only (heads-equal fast path) and demonstrate the materialized-state-snapshot semantics the apply-time check consumes. Documented limitation: the Branch 1 conflation of `Removed → NeverMember` happens because the materialized set has no row for a removed signer; the prefix walk (Branch 3) recovers the distinction when receivers are slightly behind senders. The apply-time check treats both as rejection, so the practical security outcome is identical on this path.
* End-to-end DAG-level integration tests for the prefix walk (encrypted op construction, keyring setup, ancestor chains) are **deferred** — not in scope for #2328. The per-transition resolver tests cover the state-machine half; the BFS-truncation half is exercised in production by the existing `group-3node` and partition-heal e2e suites.

**Side benefits delivered with #2328** (carried in the same PR):
* Stripped internal roadmap codes (`B1`, `B2`, `B3`, `S13`, `C4`, `D1`) from code comments and log messages — they confuse readers without the RFC context. Replaced with descriptive shortforms ("cross-DAG check", "governance-pending drain", "apply-time membership check", "forward-only invariant", "deny-list"). The RFC remains the authoritative spec for the design rationale.
* Mechanical-substitution cleanup pass: every `"<code>: "` log prefix was rewritten with proper spacing after the colon (PR-review feedback caught `"B2:"` → `"governance-pending drain:"` dropping the trailing space).

**Acceptance criteria status**:
- ✅ All three call sites of the validity check use forward-only — audited and documented inline.
- ✅ Property test + canary cover the prefix-boundary sensitivity (resolver state-machine half).
- ⚠️ e2e partition-heal harness deferred — covered by existing `group-3node` and partition tests in production but not added as a dedicated forward-only e2e.
- ✅ Architecture doc lives at the load-bearing block in `membership_status.rs` (the single source of truth for the implementer); the `docs/architecture/membership-and-leave.md` update is **left as a follow-up** to keep #2328 focused on code + tests.

**References**: §2 (architectural approach + decisions 1–6), [#2328](https://github.com/calimero-network/core/pull/2328) (forward-only tests + doc block), taint-cascade scenario (the failure mode forward-only prevents)
...
#### D1 — Network-layer deny-list keyed on signer identity *(narrow scope, DoS reduction)* ✅ DONE

**Phase**: 6 · **Size**: S · **Depends on**: — (independent; can ship before B3) · **Blocks**: — · **Status**: landed in [#2329](https://github.com/calimero-network/core/pull/2329).

**As-shipped notes**:
* **Scope deviation from the original spec**: shipped as a **state-delta** entry-point filter (not governance-op filter). The state-delta path was the right place after all — even with B3 authoritative, the deny-list short-circuits the buffer/drain/check pipeline for traffic from peers we've already removed (saves the governance-pending drain pass, the membership lookup, and the prefix walk). Governance-op deny is implicit via the existing role-gated apply path, and encrypted-by-default RootOps (D5) will close the parallel surface there.
* **Storage**: per-group key at prefix `0x3B` (`GroupDeniedMember`), shape mirrors `GroupMember`. Per-group rather than per-peer-id keying is the correct granularity — the same identity can be a member of multiple groups, and connection-level (libp2p) gating on peer-id would drop legitimate traffic for the groups the member still belongs to. Each context has its own gossip topic, so the deny set is scoped to exactly the contexts where the member was removed.
* **Apply hooks** in `apply_group_op_mutations`: `MemberAdded` / `MemberJoinedViaTeeAttestation` → `clear_denied`; `MemberRemoved` / `MemberLeft` → `mark_denied` (with namespace-leave cascading to every group the member belonged to). `mark_denied` carries a `debug_assert!` that the member has been removed from the materialized set before the deny write, enforcing the caller contract in dev / test without cost in release.
* **Persistence + restart**: deny-list entries live in the regular store column, so survive restarts automatically.
* **Group teardown**: `clear_all_denied` sweep helper added to `delete_group_local_rows`. The scan uses `PublicKey::from([0u8; 32])` as the seek start — `[0u8; 32]` is the lexicographic minimum of the 32-byte identity space (no `PublicKey` can sort before it) and RocksDB uses byte-wise comparison, matching the convention used by `delete_all_member_capabilities` and the other per-group sweep helpers. Documented inline at the call site.
* **Entry-point filter** in `handle_state_delta` calls a single `is_author_denied_for_context(store, context_id, author)` that encapsulates the `get_group_for_context` → `is_denied` two-step lookup. Returns `Ok(false)` for non-group contexts (nothing to deny on). Failures fall through to the cross-DAG check rather than dropping a legitimate delta on a transient store error; the failure is logged at warn level so storage corruption affecting the deny prefix surfaces in monitoring instead of silently degrading the defense-in-depth layer.
* **Drain-path asymmetry — intentional**: `drain_governance_pending` calls `apply_authorized_state_delta` directly and bypasses the entry-point deny-list filter. A buffered delta carries the sender's pre-buffering `governance_position`; the cross-DAG check (with the C4 forward-only invariant) correctly resolves to `Member` against that position, which is the authoritative outcome. The deny-list is an entry-point optimization for post-removal *new* traffic; the drain path's authorization is decided by the cross-DAG check against the original position. Documented inline at the deny-list call site so the next reader doesn't re-litigate the asymmetry.
* **Atomicity limitation — documented as a known constraint**: the store API has no transactional batching primitive; each `.put` / `.delete` is independent. A process crash between `remove_group_member` and `mark_denied` leaves the deny entry stale-not-written; a crash between `add_group_member` and `clear_denied` leaves the deny entry stale-still-set. The cross-DAG check is authoritative for safety in the first case; the second case requires another `MemberAdded` apply to clear and is bounded by that recovery path. Not worked around — no actual fix is available at the current store layer.
* **Future-tracked**: reset hook for C3 (`MemberRestored`) — the `clear_denied` primitive exists; the C3 op type itself is still on the C-track.

**Test coverage**: 10 unit / integration tests covering the store-level primitives (mark/clear/is_denied/clear_all), idempotency, per-group scoping, the Add→Remove→Add cycle ending cleared, apply-path integration via `apply_local_signed_group_op`, and the teardown sweep. Plus the `local_state_join_tracking_and_delete_group_rows_cleanup` extension that seeds two deny rows and asserts both are cleared after teardown (single-row test would let a non-prefix-sweep regression through). Prefix-collision guard test (`distinct_prefixes`) extended with `GROUP_DENIED_MEMBER_PREFIX` (and `GROUP_KEY_PREFIX`, which was already missing).

**Acceptance criteria status**:
- ✅ Removed member's state deltas dropped at the entry point on every peer that has applied `MemberRemoved` / `MemberLeft`.
- ✅ Apply-path integration tests pin Remove → mark, Add → clear, Add→Remove→Add → cleared.
- ✅ Deny-list survives node restart (regular store column, no special handling).
- ⚠️ Bypass test (rotate libp2p peer-id, still dropped) — implicit: the deny key is `(group_id, identity_pubkey)`, not `peer_id`. Libp2p peer-id rotation has no effect on the deny check. Not added as an explicit e2e but the keying choice rules it out by construction.
- ⚠️ Reset for `MemberRestored` (C3) — `clear_denied` exists and is wired to `MemberAdded`. C3's own op type is still pending.

**References**: §2 decisions 8, 9, 20, [#2329](https://github.com/calimero-network/core/pull/2329) (deny-list)
