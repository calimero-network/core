...
**Task A0 decides this**: read how an entity's `own_hash`/`merkle_hash` is computed (`crates/storage/src/index.rs` around `merkle_hash`/`own_hash`, and the `Element`/`Entry` hash in `crates/storage/src/entities.rs`) and determine whether `metadata.crdt_type` is an input. If **not** an input → use **Model S**. If it **is** an input → use **Model O**. The rest of the plan is written for **Model S** with the Model-O deltas called out in each task.
