...
#[ic_cdk::query]
pub fn get_proposal(proposal_id: ICProposalId) -> Option<ICProposal> {
    PROXY_CONTRACT.with(|contract| {
        let contract = contract.borrow();
...
#[ic_cdk::query]
pub fn get_proposal_approvals(proposal_id: ICProposalId) -> Option<ICProposalWithApprovals> {
    PROXY_CONTRACT.with(|contract| {
        let contract = contract.borrow();
    ...
    PROXY_CONTRACT.with(|contract| {
        ...
        if let Some(approvals) = contract.approvals.get(&proposal_id) {
            if approvals.contains(&signer_id) {
                return Some(ICProposalApprovalWithSigner {
                    proposal_id,
                    signer_id,
