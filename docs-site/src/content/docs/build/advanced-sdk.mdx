...
<SeqDiagram
  ...
  steps={[
    ...
    { from: 1, to: 2, label: '__calimero_merge_root_state(local, incoming)', color: '#8b5cf6' },
    { from: 2, to: 2, label: 'deserialize as T; run Mergeable::merge', color: '#8b5cf6' },
    { from: 2, to: 1, label: 'merged root bytes', dashed: true, color: '#8b5cf6' },
...
Calimero's upgrade gate (and the `calimero-abi diff` CI lint) compare the old and
