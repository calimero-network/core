...
|-------|------|------------|--------------|
| `#[app::state]` | attribute | struct / enum | Declares the persistent application state. |
...
Declares the single persistent state struct. State is built from
[CRDT collections](/build/collections/) so concurrent edits merge
deterministically. The macro accepts optional parameters:
...
- `version = N` — a schema version, forwarded to `#[derive(app::Migrate)]` for
  migrations.
- `storage_key` — a custom storage key for the root.
