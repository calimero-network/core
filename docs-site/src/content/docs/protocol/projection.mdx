...
## A concrete fold: three ops to one root

Take three data-plane ops in one scope. The entity ids are derived from their keys, so they are the same on every node (hashes abbreviated and illustrative):
...
Folding these — in *any* order — gives one materialized state. `op3` carries a higher stamp than `op1` on slot `0x9a…m1`, so the delete wins and `m1` is absent; a late-arriving `op1` cannot resurrect it (lower stamp loses):
...
Only `m2` survives, so `entities_hash` is computed over that single live entity, in ascending-id order:
...
A node that applied `op3, op1, op2` and a node that applied `op1, op2, op3` walk different paths but resolve the same slots to the same bytes, so both land on `scope_root = 0x8d27…f0` — equal sets, equal root, converged. Add a fourth op that only rotates an object's writers and the *data* slots above are byte-identical, yet `acl_hash` moves and the `scope_root` changes with it — the property the [Key property](#the-scope-root) note above relies on.
