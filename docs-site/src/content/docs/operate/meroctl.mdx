...
| `create`                           | Create a new context.                             | `--application-id`, `--group-id` (required), `--params`, `--watch <path>`, `--metadata`, `--context-seed`, `--as` (identity alias), `--name` (context alias), `--service`, `--identity-secret`, `--group-name` |
...
| `grant`                            | Grant a permission to an identity.                | identity, capability, `--context`, `--as`                   |
| `revoke`                           | Revoke a permission from an identity.             | identity, capability, `--context`, `--as`                   |
...
<Aside type="caution">
  The blob `upload`/`download`/`info`/`delete` flag names above (`--file`,
  `--output`, `--blob-id`, `--context-id`) are taken from the command help text.
  Confirm the exact long-flag spelling with `meroctl blob <subcommand> --help`.
</Aside>
...
Call a method on a context. The method runs as a mutation by default; pass an
identity to run a view as that identity.
...
| `--as <KEY>` (`--substitute`) | alias list       | —           | Aliases to substitute in the payload (use `{alias}`).          |
