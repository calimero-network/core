...
| --- | --- | --- | --- |
| `mode` | `"Standard"` \| `"ReadOnly"` | `"Standard"` | Node role. `ReadOnly` disables JSON-RPC execution and is used for observer / TEE nodes. |
...
| Key | Type | Default | Meaning |
| --- | --- | --- | --- |
| `listen` | array of multiaddr | `["/ip4/0.0.0.0/tcp/2428", "/ip4/0.0.0.0/udp/2428/quic-v1"]` | Addresses the swarm binds for inbound peer connections. TCP and QUIC are both seeded by `merod init`. Default port is `2428`. |
...
| Key | Type | Default | Meaning |
| --- | --- | --- | --- |
| `listen` | array of multiaddr | `["/ip4/127.0.0.1/tcp/2528"]` | Addresses the HTTP server binds. |
...
| `check_interval` | duration (seconds) | non-zero default | Interval between compaction sweeps. Serialized in seconds. |
