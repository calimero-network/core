...
cp res/migration_suite_v3_add_list.wasm res/bundle-temp/app.wasm

if [ -f res/abi.json ]; then
    cp res/abi.json res/bundle-temp/abi.json
fi

WASM_SIZE=$(stat -f%z res/migration_suite_v3_add_list.wasm 2>/dev/null || stat -c%s res/migration_suite_v3_add_list.wasm 2>/dev/null || echo 0)
