...
cp $TARGET/wasm32-unknown-unknown/$PROFILE/scenario_authored_vector_v2.wasm ./res/

if [ "$PROFILE" = "app-release" ] && command -v wasm-opt > /dev/null; then
  wasm-opt -Oz --enable-bulk-memory ./res/scenario_authored_vector_v2.wasm -o ./res/scenario_authored_vector_v2.wasm
fi
