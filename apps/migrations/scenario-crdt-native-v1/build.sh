...
cp $TARGET/wasm32-unknown-unknown/$PROFILE/scenario_crdt_native_v1.wasm ./res/

if [ "$PROFILE" = "app-release" ] && command -v wasm-opt > /dev/null; then
  wasm-opt -Oz --enable-bulk-memory ./res/scenario_crdt_native_v1.wasm -o ./res/scenario_crdt_native_v1.wasm
fi
