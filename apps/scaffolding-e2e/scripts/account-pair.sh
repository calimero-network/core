...
if forged=$(api_post "${holder}" "namespaces/${namespace}/account/pair-complete" \
    "{\"deviceId\":\"${device}\",\"kemPublicKey\":\"${forged_kem}\",\"signPublicKey\":\"${sign}\",\"statement\":\"${statement}\"}" 2>/dev/null) \
...
complete=$(api_post "${holder}" "namespaces/${namespace}/account/pair-complete" \
    "{\"deviceId\":\"${device}\",\"kemPublicKey\":\"${kem}\",\"signPublicKey\":\"${sign}\",\"statement\":\"${statement}\"}")
...
# The comparison a person makes, made here because this script is the only thing
# standing on both ends of the exchange. Equal codes mean the holder certified
# the key material the new device actually minted.
