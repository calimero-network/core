...
A simple Access Control application built with Calimero SDK to demonstrate the usage of context management host functions.
...
cargo build -p access-control --target wasm32-unknown-unknown
```
...
```bash
calimero-abi extract target/wasm32-unknown-unknown/debug/access_control.wasm -o apps/access-control/res/abi.json
```
