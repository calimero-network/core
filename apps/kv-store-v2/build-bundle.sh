...
# Copy WASM file
cp res/kv_store_v2.wasm res/bundle-temp/app.wasm
...
WASM_SIZE=$(stat -f%z res/kv_store_v2.wasm 2>/dev/null || stat -c%s res/kv_store_v2.wasm 2>/dev/null || echo 0)
...
# Sign the manifest with the SAME test key as v1 (AppKey continuity)
...
cd res/bundle-temp
tar -czf ../kv-store-2.0.0.mpk manifest.json app.wasm abi.json 2>/dev/null || \
tar -czf ../kv-store-2.0.0.mpk manifest.json app.wasm 2>/dev/null

# Cleanup
cd ..
rm -rf bundle-temp

echo "Bundle created: res/kv-store-2.0.0.mpk"
