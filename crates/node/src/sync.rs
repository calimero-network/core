...
impl Node {
    ...
    //     else {
    //         let message = to_json_vec(&DirectMessage::Error(BlobError::ApplicationNotFound {
    ...
    //         let message = to_json_vec(&DirectMessage::Error(BlobError::ContextNotFound {
    ...
    //     let data = to_json_vec(&DirectMessage::BlobRequest(request))?;
    ...
    //                     DirectMessage::BlobChunk(chunk) => {
    //                         tx.send(chunk).await?;
    //                     }
    //                     message @ (DirectMessage::ActionsBatch(_)
    //                     | DirectMessage::SyncRequest(_)
    //                     | DirectMessage::BlobRequest(_)) => {
    //                         warn!("Ignoring unexpected message: {:?}", message);
    //                     }
    //                     DirectMessage::Error(err) => {
    ...
    //     let data = to_json_vec(&DirectMessage::SyncRequest(request))?;
    ...
    //             Ok(message) => match message {
    //                 Some(message) => match from_json_slice(&message?.data)? {
    //                     DirectMessage::ActionsBatch(batch) => {
    //                         self.apply_actions_batch(chosen_peer, context, batch)
    //                             .await?;
    //                     }
    //                     message @ (DirectMessage::BlobChunk(_)
    //                     | DirectMessage::SyncRequest(_)
    //                     | DirectMessage::BlobRequest(_)) => {
    //                         warn!("Ignoring unexpected message: {:?}", message);
    //                     }
