...
impl BlobCommand {
    pub async fn run(&self, node_client: &NodeClient) -> Result<()> {
        ...
        match &self.subcommand {
            ...
            BlobSubCommand::Info { blob_id } => {
                match node_client.get_blob_info(*blob_id).await {
                    Ok(Some(metadata)) => {
                        println!(
                            "  {:<44} | {:<22} | {:<20} | {}",
                            "ID".cyan(),
            ...
            BlobSubCommand::Delete { blob_id } => {
                match node_client.delete_blob(*blob_id).await {
                    Ok(deleted) => {
                        if deleted {
                            println!("{ind} Successfully deleted blob '{}'", blob_id.cyan());
                        } else {
                            println!("{ind} Blob '{}' not found or already deleted", blob_id.cyan());
                        }
                    }
                    Err(err) => {
