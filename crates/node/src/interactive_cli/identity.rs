...
impl IdentityCommand {
    pub fn run(self, node: &Node) -> EyreResult<()> {
        ...
        match &self.subcommand {
            IdentitySubcommands::Ls { context_id } => {
                ...
                self.list_identities(node, &ContextId::from(context_hash), &ind.to_string())?;
            }
            IdentitySubcommands::New => {
                self.create_new_identity(node, &ind.to_string());
            }
            IdentitySubcommands::Alias { command } => {
                self.handle_alias_command(node, command, &ind.to_string())?;
            }
        }
    ...
    fn list_identities(&self, node: &Node, context_id: &ContextId, ind: &str) -> EyreResult<()> {
    ...
    fn create_new_identity(&self, node: &Node, ind: &str) {
    ...
    fn handle_alias_command(
        ...
        match command {
            AliasSubcommands::Add {
                ...
                let alias = Alias::try_from(name.clone())?;

                let resolved_context_id = match context_id {
                    Some(id) => {
                        let context_hash = resolve_identifier(node, id, Kind::Context, None)?;
                        Some(ContextId::from(context_hash))
                    }
                    None => None,
                };

                let identity_hash =
                    resolve_identifier(node, identity, Kind::Identity, resolved_context_id)?;

                let mut store = node.store.handle();
                let key = if let Some(context_id) = resolved_context_id {
                    AliasKey::identity(context_id, alias)
                } else {
                    AliasKey::context(alias)
                };
