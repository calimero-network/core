...
impl SyncManager {
    ...
    async fn internal_handle_opened_stream(&self, stream: &mut Stream) -> eyre::Result<Option<()>> {
        ...
        match payload {
            ...
            // Streams are only used for: KeyShare, BlobShare, DeltaRequest, DagHeadsRequest, IdentityRequest
            ...
            InitPayload::IdentityRequest {
                context_id: requested_context_id,
                identity,
            } => {
                // Handle identity request from peer
                self.handle_identity_request(requested_context_id, identity, stream, nonce)
                    .await?
            }
        };
