...
impl SecureStream {
    ...
    pub async fn authenticate_p2p(
        ...
        let Some(ack) = super::stream::recv(stream, None, timeout_budget).await? else {
    ...
    pub async fn prove_identity(
        ...
        // Receive challenge
        let Some(msg) = super::stream::recv(stream, None, timeout).await? else {
    ...
    pub async fn send(&mut self, message: &StreamMessage<'_>) -> eyre::Result<()> {
        match self {
            SecureStream::Authenticated {
                ...
                super::stream::send(stream, message, Some((*shared_key, *our_nonce))).await?;
                *our_nonce = thread_rng().gen(); // Rotate nonce after each send
                Ok(())
            }
            SecureStream::Protocol { stream, .. } => {
                super::stream::send(stream, message, None).await
            }
        }
    }
    ...
    pub async fn recv(&mut self, timeout: Duration) -> eyre::Result<Option<StreamMessage<'static>>> {
        match self {
            SecureStream::Authenticated {
                ...
                let msg = super::stream::recv(stream, Some((*shared_key, *their_nonce)), timeout).await?;
                if msg.is_some() {
            ...
            SecureStream::Protocol { stream, .. } => {
                super::stream::recv(stream, None, timeout).await
            }
        }
    }
    ...
    async fn authenticate_as_initiator(
        ...
        let Some(msg) = super::stream::recv(stream_ref, None, timeout).await? else {
