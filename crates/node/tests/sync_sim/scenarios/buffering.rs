...
fn make_insert_op(entity_id: u64, value: &str) -> StorageOp {
    StorageOp::Insert {
        ...
        metadata: EntityMetadata::new(CrdtType::lww_register(), entity_id * 100),
    }
}
...
mod tests {
    ...
    fn test_fifo_order_last_writer_wins() {
        ...
        for i in 1..=3 {
            ...
            let operations = vec![StorageOp::Update {
                ...
                metadata: EntityMetadata::new(CrdtType::lww_register(), i * 100),
            }];
            rt.schedule_gossip_delta(
                node_id.clone(),
                delta_id,
    ...
    fn test_convergence_blocked_with_buffered_deltas() {
        ...
        let metadata = EntityMetadata::new(CrdtType::lww_register(), 100);
