...
fn test_i4_overlapping_convergence() {
    ...
    alice.insert_entity_with_metadata(
        EntityId::from_u64(1),
        b"alice-only".to_vec(),
        EntityMetadata::new(CrdtType::lww_register(), 50),
    );

    bob.insert_entity_with_metadata(
        EntityId::from_u64(2),
        b"bob-only".to_vec(),
        EntityMetadata::new(CrdtType::lww_register(), 60),
    );
    ...
    for (i, node) in nodes.iter_mut().enumerate() {
        for j in 0..3 {
            ...
            node.insert_entity_with_metadata(
                ...
                EntityMetadata::new(CrdtType::lww_register(), (i * 1000 + j * 100) as u64),
            );
        }
        // Force HashComparison
...
fn test_i4_concurrent_modification_convergence() {
    ...
    bob.insert_entity_with_metadata(
        ...
        EntityMetadata::new(CrdtType::lww_register(), 300), // Newer
    );
...
fn create_diverged_pair(name1: &str, name2: &str, seed: u64) -> (SimNode, SimNode) {
    ...
    for i in 0..5 {
        node2.insert_entity_with_metadata(
            ...
            EntityMetadata::new(CrdtType::lww_register(), i * 100 + 50),
        );
    }
