...
impl ValidationState {
    fn should_validate(&self, parents: &[[u8; 32]]) -> bool {
        // Cascaded: single parent matches last applied
        if parents.len() == 1 {
...
impl ValidationTrackingApplier {
    ...
    async fn prepare(&self, heads: Vec<[u8; 32]>) {
...
async fn test_genesis_delta_is_linear_base() {
    ...
    // Pre-apply heads: empty (genesis case)
    applier.prepare(dag.get_heads()).await;
...
/// the last_applied_delta_id tracking.
#[tokio::test]
