...
impl NodeClient {
    ...
    pub async fn announce_blob_to_network(
        ...
        // Same withholding policy the kad announce applies: in registry-only
        // mode this node does not serve application bytecode, so it must not
        // invite an availability node to come and fetch it either. Master added
        // that rule on the kad path while this path was being written, and an
        // announce is an invitation to fetch, so it is bound by it too.
        if !self.may_share_blob(blob_id)? {
            return Ok(());
        }
