...
pub async fn handler(
    ...
    info!(group_id=%group_id_str, context_id=%context_id, signer_id=%request.signer_id, count=%request.capabilities.len(), "Granting capabilities");
    ...
    match result {
        Ok(()) => {
            info!(group_id=%group_id_str, context_id=%context_id, signer_id=%request.signer_id, "Capabilities granted successfully");
            ApiResponse { payload: () }.into_response()
        }
        Err(err) => {
            error!(group_id=%group_id_str, context_id=%context_id, signer_id=%request.signer_id, error=?err, "Failed to grant capabilities");
