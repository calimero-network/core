...
use calimero_server_primitives::registry::InstallAppFromRegistryRequest;
...
pub async fn handler(
    ...
    Json(req): Json<InstallAppFromRegistryRequest>,
) -> impl IntoResponse {
    info!(app_name=%req.app_name, registry_name=%req.registry_name, "Installing app from registry");
    ...
    // Fetch app manifest from registry
    ...
    // Install the application using existing node client
        ...
        .await
    {
        Ok(app_id) => app_id,
        Err(err) => {
            error!(app_name=%req.app_name, error=?err, "Failed to install application");
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    info!(app_name=%req.app_name, application_id=%application_id, "App installed from registry successfully");
    ApiResponse {
