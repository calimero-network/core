...
| `payload_from_group_op(group: ContextGroupId, op: &GroupOp) -> Option<OpPayload>` | fn | Membership plane (per-group ops, already decrypted): maps auth-relevant `GroupOp` variants to `MemberAdded`/`MemberRemoved`/`AdminChanged`/`DeviceLinked`/`DeviceRevoked`/`DeviceDescoped`/`AccountKeysRotated`/`DefaultCapabilitiesSet`/`MemberCapabilitySet`/`SubgroupVisibilitySet`; everything else -> `None`. The two scope-bearing arms verify the root-signed statement here, since the payload carries only its epoch |
