...
use calimero_account::{verify_device_cert, AccountGenesis, AccountId, DeviceCert, RootKeyHandoff};
...
#[must_use]
pub fn join_credential_binds(
    member: &AccountId,
    genesis: &AccountGenesis,
    chain: &[RootKeyHandoff],
    cert: &DeviceCert,
) -> bool {
    cert.account == *member && verify_device_cert(cert.account, genesis, chain, cert).is_ok()
}
...
#[must_use]
pub fn join_credential_certifies(
    member: &PublicKey,
    genesis: &AccountGenesis,
    chain: &[RootKeyHandoff],
    cert: &DeviceCert,
) -> bool {
    cert.sign_pk == *member && verify_device_cert(cert.account, genesis, chain, cert).is_ok()
}
...
pub(crate) fn credential_binds_the_member(op: &RootOp) -> bool {
    match op {
        ...
        | RootOp::MemberJoinedOpen {
            member, account, ..
        } => join_credential_binds(member, &account.genesis, &account.chain, &account.cert),
