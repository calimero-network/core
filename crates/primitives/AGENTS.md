...
- **`CrdtType`'s string payloads are vestigial and written empty**: `inner_type`, `key_type`, `value_type`, `element_type` are never read back (the `"Opaque"` and `"JsRoot"` sentinels on `LwwRegister` are the only exceptions), and production writes `""` into all of them. They used to carry `std::any::type_name` output, whose rendering rustc does not guarantee - and these bytes land in persisted `EntityIndex` metadata, in `LeafMetadata` on the wire, and in the `CausalDelta::compute_id` preimage. `borsh_encodings_are_byte_frozen` pins every variant's encoding and `legacy_type_name_payloads_still_decode` pins that old rows with populated payloads still decode; the fields stay because removing them is an ungated wire break.
