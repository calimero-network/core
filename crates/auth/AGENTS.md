...
- **`list-own` is narrower than `list`, one direction only**: `context:list-own` / `namespace:list-own` gate the three caller-scoped reads (`GET /admin-api/contexts`, `GET /admin-api/contexts/:id`, `GET /admin-api/namespaces`), whose handlers narrow the answer to the caller's own groups. The wide `list` **satisfies** the narrow one, so every pre-existing operator or client token still reaches those routes; the reverse must never hold, because `context:list` also gates `/contexts/:id/identities`, `/identities-owned`, `/storage`, `/group` and the two `for-application` listings, none of which is caller-scoped. If you widen the hierarchy in `types.rs::satisfies`, that is the pair to leave alone.
