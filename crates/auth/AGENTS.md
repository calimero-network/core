...
| `src/providers/impls/user_password.rs` | The only shipped `AuthProvider`; existing-user verification and the legacy key-id migration live here |
...
- **`KeyManager::get_key` hides revoked/expired keys; use `get_key_including_invalid` when the difference matters**: `get_key` returns `None` for a revoked or expired key rather than `Some(key)` with `is_valid() == false`, so through *that* method you cannot distinguish "never existed" from "revoked". This is deliberate — most call sites (permission edits, provider lookups, delete/rotation) only ever want a usable key. The token-verification paths (`TokenManager::verify_token_string`/`generate_token_pair`, the proxy `validate_handler`) instead call `KeyManager::get_key_including_invalid` and branch explicitly: absent → `InvalidToken` (401), revoked → `TokenRevoked` (403), key-expired → `TokenExpired` (401). Without this, `TokenRevoked` was unreachable and every revoked token surfaced as a generic 401 (issue #3069).
