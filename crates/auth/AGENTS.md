...
- **An `account_proof` session is authentication, never membership.** Its `key_id` (and so the JWT `sub`, and so `X-Auth-User`) is the **account**; whether that account may touch any given context is an at-cut question this service cannot see and must be answered per request at the node. `session_permissions` therefore defaults to `context:intent` alone - already gated twice by the warrant and `CAN_AUTHOR_ON_BEHALF` - and deliberately grants no reads until that per-request check exists.
