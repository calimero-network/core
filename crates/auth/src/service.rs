...
use axum::{
    body::Body, extract::{Path, Request, Extension, Query}, http::{HeaderMap, StatusCode}, middleware::{from_fn, Next}, response::{IntoResponse, Response}, routing::{delete, get, post, put}, Json, Router
};
...
use crate::storage::{Storage, RootKey, ClientKey, Permission, prefixes, serialize, deserialize};
...
pub async fn start_server(
    ...
    let cors_layer = if config.cors.allow_all_origins {
        ...
        // Add allowed methods
        let methods: Vec<axum::http::Method> = config.cors.allowed_methods
            .iter()
    ...
    let app: Router = Router::new()
        ...
        .route("/auth/keys/:key_id/clients/:client_id", delete(delete_client_handler))
        
        // Permission management
        .route("/auth/permissions", get(list_permissions_handler))
        .route("/auth/keys/:key_id/permissions", get(get_key_permissions_handler))
        .route("/auth/keys/:key_id/permissions", put(update_key_permissions_handler))
...
async fn token_handler(
    ...
    if token_request.auth_method == "near_wallet" {
        ...
        let near_provider = state.0.auth_service.providers()
            .iter()
            .find(|p| p.name() == "near_wallet")
            .ok_or_else(|| AuthError::AuthenticationFailed("NEAR wallet provider not found".to_string()));
        
        if let Ok(provider) = near_provider {
            // Create headers to pass to the provider
            let mut headers = HeaderMap::new();
            headers.insert("x-near-account-id", token_request.wallet_address.clone().unwrap_or_default().parse().unwrap());
            headers.insert("x-near-public-key", token_request.public_key.parse().unwrap());
            headers.insert("x-near-signature", token_request.signature.parse().unwrap());
            
            // Use as_ref() here to avoid moving the message
            headers.insert("x-near-message", token_request.message.as_ref().unwrap_or(&String::new()).parse().unwrap());
                    ...
                    match verifier.verify().await {
                        Ok(auth_response) => {
                            ...
                            if let Some(key_id) = auth_response.key_id.as_ref() {
                                ...
                                match state.0.token_generator.generate_token_pair(
                                    &client_id,
                                    key_id,
    ...
    match state.0.auth_service.authenticate_token_request(&token_request).await {
        Ok(auth_response) => {
            if !auth_response.is_valid {
                return (StatusCode::UNAUTHORIZED, Json(serde_json::json!({
                    "error": "Authentication failed"
...
async fn validate_handler(
    ...
    match state.0.auth_service.verify_token_from_headers(&headers).await {
        ...
        Err(_) => {
            (StatusCode::UNAUTHORIZED, HeaderMap::new(), "Unauthorized").into_response()
        }
    }
}
...
async fn callback_handler(
    ...
    (StatusCode::OK, "OAuth callback - implement with your OAuth provider")
}
...
async fn create_key_handler(
    ...
    match serialize(&root_key) {
        Ok(data) => {
            match state.0.storage.set(&key, &data).await {
                Ok(_) => {
...
async fn create_client_handler(
    ...
    match state.0.storage.get(&root_key_key).await {
        Ok(Some(_)) => {
            ...
            let expires_at = request.expires_in.map(|secs| {
...
async fn identity_handler(
    state: Option<Extension<Arc<AppState>>>,
) -> impl IntoResponse {
...
async fn challenge_handler(
    ...
    let message = format!("Calimero Authentication Request {}:{}", timestamp, generate_random_challenge());
