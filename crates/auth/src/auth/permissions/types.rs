...
impl Permission {
        ...
        match parts.get(0)? {
            ...
            &"application" => {
                ...
                match parts.get(1).map(|&s| s) {
                    Some("list") => Some(Permission::Application(ApplicationPermission::List(scope))),
                    Some("install") => Some(Permission::Application(ApplicationPermission::Install(scope))),
                    Some("uninstall") => Some(Permission::Application(ApplicationPermission::Uninstall(scope))),
            ...
            &"context" => {
                match (parts.get(1), parts.get(2)) {
                    ...
                    (Some(&"alias"), Some(&action)) => {
                        match action {
                            "create" => Some(Permission::Context(ContextPermission::Alias(AliasPermission::Create))),
                            "delete" => Some(Permission::Context(ContextPermission::Alias(AliasPermission::Delete))),
                            "lookup" => {
                                let context_id = parts.get(3).map(|s| s.to_string());
                                let user_id = parts.get(4).map(|s| s.to_string());
                                Some(Permission::Context(ContextPermission::Alias(AliasPermission::Lookup {
                                    context_id,
                                    user_id,
                                })))
                            },
                    ...
                    _ => {
                        ...
                        match parts.get(1).map(|&s| s) {
                            Some("create") => Some(Permission::Context(ContextPermission::Create(scope))),
                            Some("list") => Some(Permission::Context(ContextPermission::List(scope))),
                            Some("delete") => Some(Permission::Context(ContextPermission::Delete(scope))),
                            Some("leave") => Some(Permission::Context(ContextPermission::Leave(scope, user_scope))),
                            Some("invite") => Some(Permission::Context(ContextPermission::Invite(scope, user_scope))),
                            Some("execute") => {
                                let method = parts.get(4).map(|s| s.to_string());
                                Some(Permission::Context(ContextPermission::Execute(scope, user_scope, method)))
                            }
    ...
    pub fn to_string(&self) -> String {
        match self {
            ...
            Permission::Application(app_perm) => match app_perm {
                ...
                ApplicationPermission::Uninstall(scope) => format!("application:uninstall{}", scope),
            },
            ...
            Permission::Context(ctx_perm) => match ctx_perm {
                ...
                ContextPermission::Invite(scope, user) => format!("context:invite{}{}", scope, user),
    ...
    pub fn satisfies(&self, required: &Permission) -> bool {
        match (self, required) {
            ...
            (Permission::Application(ApplicationPermission::All), Permission::Application(_)) => true,
            (Permission::Application(held), Permission::Application(req)) => match (held, req) {
                ...
                (ApplicationPermission::Install(h_scope), ApplicationPermission::Install(r_scope)) => 
                    matches_scope(h_scope, r_scope),
                (ApplicationPermission::Uninstall(h_scope), ApplicationPermission::Uninstall(r_scope)) => 
                    matches_scope(h_scope, r_scope),
                _ => false,
            },
            ...
            (Permission::Blob(held), Permission::Blob(req)) => match (held, req) {
                (BlobPermission::Add(h_add), BlobPermission::Add(r_add)) => matches!(h_add, AddBlobPermission::All) 
                    || h_add == r_add,
            ...
            (Permission::Context(ContextPermission::All(h_scope)), Permission::Context(req)) => 
                matches!(req, ContextPermission::All(_)) && matches_scope(h_scope, match req {
            ...
            (Permission::Context(held), Permission::Context(req)) => match (held, req) {
                ...
                (ContextPermission::Delete(h_scope), ContextPermission::Delete(r_scope)) => 
                    matches_scope(h_scope, r_scope),
                (ContextPermission::Leave(h_scope, h_user), ContextPermission::Leave(r_scope, r_user)) => 
                    matches_scope(h_scope, r_scope) && matches_user_scope(h_user, r_user),
                (ContextPermission::Invite(h_scope, h_user), ContextPermission::Invite(r_scope, r_user)) => 
                    matches_scope(h_scope, r_scope) && matches_user_scope(h_user, r_user),
...
fn matches_alias(held: &AliasPermission, required: &AliasPermission) -> bool {
    match (held, required) {
        ...
        (AliasPermission::Lookup { context_id: held_ctx, user_id: held_user },
         AliasPermission::Lookup { context_id: req_ctx, user_id: req_user }) => {
...
mod tests {
    ...
    fn test_permission_satisfaction() {
        ...
        let app_list_specific = Permission::Application(ApplicationPermission::List(
            ResourceScope::Specific(vec!["app1".to_string(), "app2".to_string()])
        ));
        let app_list_required = Permission::Application(ApplicationPermission::List(
    ...
    fn test_alias_permission_parsing() {
        ...
        assert!(matches!(perm, Permission::Context(ContextPermission::Alias(AliasPermission::Create))));

        // Test delete permission
        let perm = Permission::from_str("context:alias:delete").unwrap();
        assert!(matches!(perm, Permission::Context(ContextPermission::Alias(AliasPermission::Delete))));

        // Test lookup permission with context
        let perm = Permission::from_str("context:alias:lookup[ctx-123]").unwrap();
        match perm {
            Permission::Context(ContextPermission::Alias(AliasPermission::Lookup { context_id, user_id })) => {
