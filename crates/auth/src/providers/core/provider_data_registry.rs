...
pub fn parse_auth_data(method: &str, value: Value) -> Result<Box<dyn Any + Send + Sync>, AuthError> {
...
pub fn get_all_auth_data_types() -> HashMap<String, Value> {
    ...
    registry
        .types
        .iter()
        .map(|(method, auth_type)| {
            (
                method.clone(),
                auth_type.get_sample_structure(),
            )
        })
        .collect()
}
...
macro_rules! register_auth_data_type {
    ($auth_type:expr) => {
        ...
        fn register_this_auth_data_type() {
            $crate::providers::core::provider_data_registry::register_auth_data_type(Box::new($auth_type));
        }
    };
} 