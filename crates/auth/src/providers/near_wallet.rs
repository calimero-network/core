...
use crate::{AuthError, AuthProvider, AuthResponse, RequestValidator, AuthRequestVerifier, AuthVerifierFn};
use crate::config::NearWalletConfig;
use crate::providers::jwt::TokenManager;
use crate::storage::{Storage, StorageError, prefixes, serialize, deserialize, RootKey};
...
impl NearWalletProvider {
    ...
    pub fn new(config: NearWalletConfig, storage: Arc<dyn Storage>) -> Self {
        Self { config, storage, token_manager: None }
    }
    ...
    pub fn with_token_manager(config: NearWalletConfig, storage: Arc<dyn Storage>, token_manager: TokenManager) -> Self {
        Self { config, storage, token_manager: Some(token_manager) }
    }
    ...
    fn extract_signature_data<B>(&self, request: &Request<B>) -> Result<(String, String, Vec<u8>), AuthError> {
        let headers = request.headers();
        
        // Extract the account ID
        let account_id = headers.get("x-near-account-id")
        ...
        let public_key = headers.get("x-near-public-key")
        ...
        let signature = headers.get("x-near-signature")
        ...
        // Extract the message
        let message = headers.get("x-near-message")
        ...
        let _signature_bytes = STANDARD.decode(signature)
            .map_err(|_| AuthError::AuthenticationFailed("Invalid NEAR signature encoding".to_string()))?;
    ...
    async fn verify_signature(&self, public_key_str: &str, message: &[u8], signature_str: &str) -> Result<bool, AuthError> {
        // Parse the public key
        let public_key = PublicKey::from_str(public_key_str)
            .map_err(|_| AuthError::AuthenticationFailed("Invalid NEAR public key format".to_string()))?;
        ...
        let signature = Signature::from_parts(KeyType::ED25519, &signature_bytes)
            .map_err(|_| AuthError::AuthenticationFailed("Invalid NEAR signature format".to_string()))?;
        
        // Verify the signature
        if !signature.verify(message, &public_key) {
            return Err(AuthError::AuthenticationFailed("Invalid NEAR signature".to_string()));
        }
    ...
    async fn verify_account_owns_key(&self, account_id: &str, public_key: &str) -> Result<bool, AuthError> {
    ...
    async fn get_root_key_for_account(&self, account_id: &str) -> Result<Option<(String, RootKey)>, AuthError> {
        ...
        match self.storage.get(&key).await {
            Ok(Some(data)) => {
                let root_key: RootKey = deserialize(&data)
                    .map_err(|err| AuthError::StorageError(format!("Failed to deserialize root key: {}", err)))?;
            ...
            Err(err) => {
                ...
                Err(AuthError::StorageError(format!("Failed to get root key: {}", err)))
            }
        }
    }
    ...
    async fn create_root_key(&self, account_id: &str, public_key: &str) -> Result<(String, RootKey), AuthError> {
        ...
        // Store the root key
        let key = format!("{}{}", prefixes::ROOT_KEY, key_id);
        let data = serialize(&root_key)
            .map_err(|err| AuthError::StorageError(format!("Failed to serialize root key: {}", err)))?;
    ...
    async fn update_last_used(&self, key_id: &str) -> Result<(), AuthError> {
        ...
        match self.storage.get(&key).await {
            Ok(Some(data)) => {
                let mut root_key: RootKey = deserialize(&data)
                    .map_err(|err| AuthError::StorageError(format!("Failed to deserialize root key: {}", err)))?;
    ...
    async fn authenticate<B>(&self, request: &Request<B>, account_id: &str, public_key: &str, message: &[u8]) -> Result<AuthResponse, AuthError>
    where
        B: Send + Sync,
    {
        // Verify the signature
        let signature = request.headers().get("x-near-signature")
            .ok_or_else(|| AuthError::AuthenticationFailed("Missing NEAR signature".to_string()))?
            .to_str()
            .map_err(|_| AuthError::AuthenticationFailed("Invalid NEAR signature".to_string()))?;
        ...
        if !self.verify_account_owns_key(account_id, public_key).await? {
            return Err(AuthError::AuthenticationFailed("Public key does not belong to account".to_string()));
        }
        ...
        if let Some(token_manager) = &self.token_manager {
            ...
            match token_manager.generate_token_pair(&client_id, &key_id, &permissions).await {
                Ok((access_token, refresh_token)) => {
                    // Store the token info in request extensions (as a HashMap) for later use
                    if let Some(extensions) = request.extensions().get::<std::collections::HashMap<String, String>>() {
...
impl<B: Send + Sync> RequestValidator<B> for NearWalletProvider {
    async fn validate_request(&self, request: &Request<B>) -> Result<AuthResponse, AuthError> {
        ...
        self.authenticate(request, &account_id, &public_key, &message).await
    }
}
...
impl AuthVerifierFn for NearWalletVerifier {
    async fn verify(&self) -> Result<AuthResponse, AuthError> {
        ...
        // Verify the signature
        self.provider.verify_signature(&auth_data.public_key, &auth_data.message, &auth_data.signature).await?;
        
        // Verify the account owns the key
        if !self.provider.verify_account_owns_key(&auth_data.account_id, &auth_data.public_key).await? {
            return Err(AuthError::AuthenticationFailed("Public key does not belong to account".to_string()));
        }
        
        // Get or create the root key
        let (key_id, _root_key) = match self.provider.get_root_key_for_account(&auth_data.account_id).await? {
            Some((key_id, root_key)) => {
                // Update the last used timestamp
            ...
            None => {
                // Create a new root key
                self.provider.create_root_key(&auth_data.account_id, &auth_data.public_key).await?
            }
        };
