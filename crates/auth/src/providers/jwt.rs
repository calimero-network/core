...
use crate::{AuthError, AuthResponse, AuthRequestVerifier, AuthVerifierFn};
use crate::config::JwtConfig;
use crate::storage::{Storage, StorageError, prefixes, ClientKey, deserialize};
...
impl TokenManager {
    ...
    async fn generate_access_token(
        ...
        let token = encode(&Header::default(), &claims, &encoding_key)
            .map_err(|err| AuthError::AuthenticationFailed(format!("Failed to generate JWT token: {}", err)))?;
    ...
    async fn generate_refresh_token(
        ...
        let encoding_key = EncodingKey::from_secret(self.config.secret.as_bytes());
        let token = encode(&Header::default(), &claims, &encoding_key)
            .map_err(|err| AuthError::AuthenticationFailed(format!("Failed to generate JWT refresh token: {}", err)))?;
    ...
    pub async fn generate_token_pair(
        ...
        let access_token = self.generate_access_token(client_id, key_id, permissions).await?;
    ...
    pub async fn refresh_token_pair(
        ...
        let token_data = decode::<Claims>(refresh_token, &decoding_key, &validation)
            .map_err(|err| AuthError::AuthenticationFailed(format!("Invalid refresh token: {}", err)))?;
        
        let claims = token_data.claims;
        
        // Validate that the token is a refresh token
        if !claims.jti.starts_with("refresh_") {
            return Err(AuthError::AuthenticationFailed("Invalid refresh token type".to_string()));
        }
        ...
        match self.storage.get(&client_key_path).await {
            Ok(Some(data)) => {
                let client_key: ClientKey = deserialize(&data)
                    .map_err(|err| AuthError::StorageError(format!("Failed to deserialize client key: {}", err)))?;
                
                // Check if the client key has been revoked
                if client_key.revoked_at.is_some() {
                    return Err(AuthError::AuthenticationFailed("Client key has been revoked".to_string()));
                }
                
                // Check if the key has expired
                if let Some(expires_at) = client_key.expires_at {
                    if expires_at < Utc::now().timestamp() as u64 {
                        return Err(AuthError::AuthenticationFailed("Client key has expired".to_string()));
                    }
                }
                
                // Generate new token pair
                self.generate_token_pair(&client_id, &key_id, &client_key.permissions).await
            }
            Ok(None) => {
                Err(AuthError::AuthenticationFailed("Client key not found".to_string()))
            }
            Err(err) => {
                error!("Failed to get client key: {}", err);
                Err(AuthError::StorageError(format!("Failed to get client key: {}", err)))
            }
        }
    }
    ...
    pub async fn verify_token_from_headers(
        ...
        let auth_header = headers.get("Authorization")
            .ok_or_else(|| AuthError::AuthenticationFailed("Missing Authorization header".to_string()))?
            .to_str()
            .map_err(|_| AuthError::AuthenticationFailed("Invalid Authorization header".to_string()))?;
        
        // Check that it's a Bearer token
        if !auth_header.starts_with("Bearer ") {
            return Err(AuthError::AuthenticationFailed("Invalid Authorization header format".to_string()));
        }
        ...
        if claims.jti.starts_with("refresh_") {
            return Err(AuthError::AuthenticationFailed("Cannot use refresh token for authentication".to_string()));
        }
