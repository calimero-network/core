...
pub struct ProviderFactory {
    providers: HashMap<String, Box<dyn Fn(Arc<dyn Storage>, &AuthConfig) -> Result<Box<dyn AuthProvider>, eyre::Error> + Send + Sync>>,
}
...
impl ProviderFactory {
    ...
    pub fn register_near_wallet(&mut self) {
        self.register("near_wallet", |storage, config| {
            ...
            let provider = near_wallet::NearWalletProvider::with_token_manager(
                near_config,
                storage,
                token_manager,
            );
    ...
    where
        F: Fn(Arc<dyn Storage>, &AuthConfig) -> Result<Box<dyn AuthProvider>, eyre::Error> + Send + Sync + 'static,
    {
        self.providers.insert(name.to_string(), Box::new(factory));
    }
    
    /// Create all enabled providers from configuration
    pub fn create_providers(&self, storage: Arc<dyn Storage>, config: &AuthConfig) -> Result<Vec<Box<dyn AuthProvider>>, eyre::Error> {
    ...
    pub fn create_provider(&self, name: &str, storage: Arc<dyn Storage>, config: &AuthConfig) -> Result<Box<dyn AuthProvider>, eyre::Error> {
...
/// Create all enabled providers from configuration
pub fn create_providers(storage: Arc<dyn Storage>, config: &AuthConfig) -> Result<Vec<Box<dyn AuthProvider>>, eyre::Error> {
