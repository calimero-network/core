...
pub trait AuthProvider: Send + Sync {
    ...
    fn verify_token_request(&self, token_request: &service::TokenRequest) -> eyre::Result<AuthRequestVerifier> {
        // Default implementation returns an error
        Err(eyre::eyre!("Token request verification not supported by this provider"))
    }
}
...
impl AuthService {
    ...
    /// 
    /// * `Result<AuthResponse, AuthError>` - The result of the verification
    pub async fn verify_and_check_permissions<B>(&self, request: &Request<B>) -> Result<AuthResponse, AuthError>
    where
        B: Send + 'static,
    {
        ...
        let empty_body_req = builder.body(Body::empty())
        ...
        Err(last_error.unwrap_or(AuthError::AuthenticationFailed("No valid authentication provider found".to_string())))
    }
    ...
    ///
    /// * `Result<AuthResponse, AuthError>` - The result of the verification
    pub async fn verify_token_from_headers(&self, headers: &HeaderMap) -> Result<AuthResponse, AuthError> {
    ...
    /// 
    /// * `Result<AuthResponse, AuthError>` - The result of the verification
    pub async fn authenticate_token_request(&self, token_request: &service::TokenRequest) -> Result<AuthResponse, AuthError> {
        // Try each provider in order
        let mut last_error = None;
