...
pub struct NetworkClient {
    network_manager: calimero_utils_actix::LazyRecipient<NetworkMessage>,
}

impl NetworkClient {
    pub const fn new(network_manager: calimero_utils_actix::LazyRecipient<NetworkMessage>) -> Self {
        Self { network_manager }
    }

    pub async fn dial(&self, peer_addr: libp2p::Multiaddr) -> eyre::Result<()> {
    ...
    pub async fn listen_on(&self, addr: libp2p::Multiaddr) -> eyre::Result<()> {
        let (tx, rx) = oneshot::channel();
    ...
    pub async fn mesh_peer_count(&self, topic: libp2p::gossipsub::TopicHash) -> usize {
        let (tx, rx) = oneshot::channel();
    ...
    pub async fn mesh_peers(&self, topic: libp2p::gossipsub::TopicHash) -> Vec<libp2p::PeerId> {
        let (tx, rx) = oneshot::channel();
