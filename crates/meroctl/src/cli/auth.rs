...
impl LoginCommand {
    pub async fn run(&self, _environment: &Environment) -> EyreResult<()> {
        ...
        let auth_result = timeout(Duration::from_secs(self.timeout), callback_rx)
            .await
    ...
    async fn start_callback_server(
        &self,
    ) -> EyreResult<(u16, oneshot::Receiver<Result<AuthCallback, String>>)> {
        ...
        let listener = listener.ok_or_eyre(format!(
