    ...
    T: ScopedAlias + UrlFragment + DeserializeOwned,
{
    if let Some(alias) = entity {
        // If entity is provided, resolve it
        lookup_alias(multiaddr, keypair, alias, scope)
            .await
            .and_then(|res| {
                res.data
                    .value
                    .ok_or_else(|| eyre!("Entity not found for alias"))
            })
    } else {
        // Otherwise, use the default alias
        let default_alias: Alias<T> = "default".parse().expect("'default' is a valid alias name");

        lookup_alias(multiaddr, keypair, default_alias, scope)
            .await
            .and_then(|res| {
                res.data
                    .value
                    .ok_or_else(||
                        eyre!("No default entity set. Please set one with appropriate 'use' command or specify entity explicitly")
                    )
            })
    }
}

// Keeping resolve_context function as a convenience wrapper
pub async fn resolve_context(
    multiaddr: &Multiaddr,
    keypair: &Keypair,
    context: Option<Alias<ContextId>>,
) -> EyreResult<ContextId> {
    resolve_entity(multiaddr, keypair, context, None)
        .await
        .map_err(|e| eyre!("Context resolution error: {}", e))
}
