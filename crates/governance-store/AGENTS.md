...
- **`ACCOUNT_ROOT_LOCK` and `NODE_DEVICE_MINT_LOCK` have a fixed order, and it is mint-then-root.** `ensure_enrolled` takes `NODE_DEVICE_MINT_LOCK` and then calls `ensure_account_root`, which takes `ACCOUNT_ROOT_LOCK`. Anything already holding the root lock must therefore NOT reach for the mint lock - that is an ABBA deadlock. `try_import_account_root` releases the replaced root's device rows while holding only the root lock for exactly this reason; it is safe because that path opens the datastore directly, which requires the node to be stopped.
