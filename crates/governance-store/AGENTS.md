...
| `node_device.rs` | `NodeDeviceRepository` - this node's own per-namespace `DeviceId` + X25519 agreement secret, and the node-level **account root** it derives them from (all node-local, never gossiped): `AccountRoot`, its BIP-39 mnemonic export/import, and `try_import_account_root` -> `ImportedRoot` |
