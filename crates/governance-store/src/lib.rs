...
pub use self::node_device::{DeviceSecret, NodeDevice, NodeDeviceRepository};
...
///
/// The all-zeros 32-byte encoding is NOT a refactor hazard despite being a
/// valid `PublicKey` byte pattern: it can never collide with a real signing
/// identity. An Ed25519 public key is `A = a·B`, a point in the prime-order
/// subgroup generated by the basepoint `B`. The all-zeros encoding decodes to
/// the curve's identity/low-order (torsion) point, which lies OUTSIDE that
/// prime-order subgroup — so no public key derived from a secret scalar can
/// ever serialize to all-zeros. There is therefore no legitimate keypair whose
/// public key equals this sentinel, and the established-check cannot be fooled
/// into treating a placeholder as a real admin (nor vice-versa). This is why we
/// keep the cheap `[u8; 32]` sentinel rather than wrapping the field in
/// `Option<PublicKey>`. The invariant "a fresh keypair never equals the
/// sentinel" is pinned by the `placeholder_admin_identity_never_equals_a_real_key`
/// unit test below.
