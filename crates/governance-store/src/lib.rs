...
pub use self::tee::{
    ...
    is_tee_authority, is_tee_authority_for_context, read_tee_admission_policy,
    read_tee_authoring_policy, tee_admission_record, tee_admission_records,
    tee_authorities_for_context, tee_authority_evidence, tee_authority_key, tee_evidence_owed,
...
/// not-yet-established namespace (placeholder admin) from an established one
/// (real founder). Defined once here so the seed and the gate cannot drift on
/// the magic value (#2474).
///
/// # Why all-zeros is a SAFE sentinel (#599)
///
/// The field these are compared against is an [`AccountId`], which is
/// `domain_hash(ACCOUNT_ID_DOMAIN, borsh(AccountRoot))` — a 32-byte SHA-256
/// digest. Producing a root that hashes to all-zeros is a full preimage
/// search, so no derivable account can collide with the sentinel and the
/// established-check cannot be fooled into treating a placeholder as a real
/// founder (nor vice-versa). That is why the cheap `[u8; 32]` sentinel is kept
/// rather than wrapping the field in `Option<AccountId>`.
///
/// This argument replaced an Ed25519 one when the field moved from a signing
/// key to an account: the old reasoning was that all-zeros decodes to a
/// torsion point outside the prime-order subgroup, which says nothing about a
/// hash output. Both arguments reach the same conclusion, but only one of them
/// is about the type actually stored here. The invariant "a freshly derived
/// account never equals the sentinel" is pinned by the
/// `placeholder_admin_identity_never_equals_a_real_key` unit test below.
...
impl<'a> GroupHandle<'a> {
    ...
    // --- Members ---
    pub fn add_member(&self, identity: &AccountId, role: GroupMemberRole) -> EyreResult<()> {
    ...
    pub fn remove_member(&self, identity: &AccountId) -> EyreResult<()> {
    ...
    pub fn is_member(&self, identity: &AccountId) -> EyreResult<bool> {
        MembershipRepository::new(self.store).is_member(&self.group_id, identity)
    }
    pub fn get_member_value(&self, identity: &AccountId) -> EyreResult<Option<GroupMemberValue>> {
        MembershipRepository::new(self.store).member_value(&self.group_id, identity)
    }
    pub fn get_member_role(&self, identity: &AccountId) -> EyreResult<Option<GroupMemberRole>> {
        MembershipRepository::new(self.store).role_of(&self.group_id, identity)
    }

    // --- Authorization ---
    pub fn is_admin(&self, identity: &AccountId) -> EyreResult<bool> {
        MembershipRepository::new(self.store).is_admin(&self.group_id, identity)
    }
    pub fn is_direct_admin(&self, identity: &AccountId) -> EyreResult<bool> {
    ...
    pub fn is_admin_or_has_capability(&self, identity: &AccountId, cap: u32) -> EyreResult<bool> {
    ...
    pub fn get_member_metadata(&self, member: &AccountId) -> EyreResult<Option<MetadataRecord>> {
    ...
    pub fn enumerate_member_metadata(&self) -> EyreResult<Vec<(AccountId, MetadataRecord)>> {
        MetadataRepository::new(self.store).enumerate_members(&self.group_id)
    }

    // --- Capabilities ---
    pub fn get_member_capability(&self, member: &AccountId) -> EyreResult<Option<u32>> {
        CapabilitiesRepository::new(self.store).member_capability(&self.group_id, member)
    }
    pub fn set_member_capability(&self, member: &AccountId, caps: u32) -> EyreResult<()> {
        CapabilitiesRepository::new(self.store).set_member_capability(&self.group_id, member, caps)
    }
    pub fn enumerate_capabilities(&self) -> EyreResult<Vec<(AccountId, u32)>> {
