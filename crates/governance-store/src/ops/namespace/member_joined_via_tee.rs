...
pub(crate) fn apply(
    ...
    policy_gate.require_tee_attestation_verifier_membership(&signer)?;
    ...
    policy_gate.admit_member_if_absent(member, role)?;
    ...
    if let Some(event) = crate::build_auto_follow_set_if_enabled(store, &group_id, member)? {
