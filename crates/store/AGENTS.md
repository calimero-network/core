...
- Synced (replicated) state: `State`, `Delta`, `Application`, `Blobs`, `Identity`, `Alias`, `Generic`, `UnifiedOp` (the new unified causal-log op-store, mid dual-write cutover with `Delta`).
- **`Group` is mixed, and reading it as plain "synced" is a trap.** Its rows travel to peers as individually signed governance ops, never as a column dump — and under three of its prefixes it holds node-local *secrets* (the namespace identity keypair, a device's X25519 agreement secret, the account root). Every reader addresses a typed key or scans one prefix, which is the only reason that coexistence is safe; a column-level snapshot/export/sync added later would ship this node's member identity and its recovery key. Read `Column::Group`'s own doc comment before touching anything that iterates it.
...
- **Distinct from `BytecodeId`/`ContentHash`.** `BlobId` hashes a chunk-id list; governance's `BytecodeId` (which exact bytecode build a group runs) names a bytecode blob, while `ContentHash` is a plain sha256 over raw file bytes, computed on every blob write and externally supplied only as the expected-hash check input - three distinct types, with no implicit conversion between any of them (assigning one to another does not compile; converting means going through raw bytes explicitly, so a swap has to be written on purpose).
