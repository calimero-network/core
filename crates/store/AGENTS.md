...
- **Distinct from `BytecodeId`/`ContentHash`.** `BlobId` hashes a chunk-id list; governance's `BytecodeId` (which exact bytecode build a group runs) is itself a `BlobId`, while `ContentHash` is a plain sha256 over raw file bytes, only ever supplied by a human - three distinct types, with no implicit conversion between any of them (assigning one to another does not compile; converting means going through raw bytes explicitly, so a swap has to be written on purpose).
