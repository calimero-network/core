...
Has CrdtType? ----No----> LWW (legacy data)
       ...
       Yes
        |
        v
is_builtin_crdt()? ----No----> WASM callback via __calimero_merge_{TypeName}
        |                      (requires #[app::mergeable] on Mergeable impl)
       Yes
        |
        v
merge_by_crdt_type() dispatch:
...
| Custom         | WASM callback to app-defined merge       |
+----------------+------------------------------------------+
```
...
For types that need app-defined merge logic (not built-in CRDTs), use `CrdtType::Custom`.

### How It Works

1. **Define your type** with custom `Mergeable` implementation
2. **Add `#[app::mergeable]`** attribute to generate WASM export
3. **Storage layer** calls WASM when entities with `CrdtType::Custom` conflict

### Example: Custom Mergeable Type
...
// The #[app::mergeable] macro generates __calimero_merge_TeamStats WASM export
#[app::mergeable]
...
| Export | Generated By | Purpose |
|--------|--------------|---------|
| `__calimero_merge_{TypeName}` | `#[app::mergeable]` | Entity-level custom merge |
| `__calimero_merge_root_state` | `#[app::state]` | Root state conflicts |
| `__calimero_alloc` | `#[app::state]` | Memory allocation for merge |

### Merge Flow for Custom Types

```
Entity with CrdtType::Custom("TeamStats") conflicts
  → merge_by_crdt_type_with_callback()
  → CrdtType::Custom("TeamStats") matched
  → RuntimeMergeCallback::merge_custom_mut("TeamStats", local, remote)
  → WASM export __calimero_merge_TeamStats called
  → Deserialize both values as TeamStats
  → TeamStats::merge() invoked
  → Serialize merged result
  → Return to storage layer
```
...
| Use Case | Solution |
|----------|----------|
| Simple key-value | `UnorderedMap<K, LwwRegister<V>>` |
| Counters | `Counter` (built-in) |
| Text editing | `ReplicatedGrowableArray` (built-in) |
| **Complex nested structures** | Custom type with `#[app::mergeable]` |
| **App-specific merge rules** | Custom type with `#[app::mergeable]` |
| **Validation during merge** | Custom type with `#[app::mergeable]` |
...
- `src/merge.rs` - Module-level documentation on merge dispatch
- `src/collections/crdt_impls.rs` - Mergeable trait implementations