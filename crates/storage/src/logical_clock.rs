...
/// Borsh-serializable wrapper around [`Timestamp`] for CRDT causal ordering.
///
/// # Ordering Invariants
///
/// `HybridTimestamp` provides total ordering suitable for conflict resolution
/// in distributed systems. The ordering satisfies these invariants:
///
/// ## Monotonicity
///
/// Timestamps generated by the same [`LogicalClock`] instance are **strictly
/// monotonically increasing**:
///
/// ```text
/// For any HLC h: h.new_timestamp() < h.new_timestamp() < h.new_timestamp() ...
/// ```
///
/// This is guaranteed even if the physical clock does not advance (e.g., when
/// generating many timestamps in rapid succession or during clock adjustments).
/// The logical counter embedded in the low 16 bits ensures monotonicity.
///
/// ## Clock Drift Tolerance
///
/// When updating from a remote timestamp via [`LogicalClock::update`], the HLC
/// will **reject timestamps more than 5 seconds in the future** relative to the
/// local physical clock. This prevents:
/// - Malicious actors from manipulating timestamps far into the future
/// - Network partitions causing excessive clock drift accumulation
/// - Accidental timestamp corruption from propagating through the system
///
/// Remote timestamps within the 5-second tolerance are accepted and update the
/// local clock to maintain causality across nodes.
///
/// ## Comparison Semantics
///
/// Timestamps can **always** be compared using standard Rust ordering traits
/// (`PartialOrd`, `Ord`). The comparison is deterministic and total:
///
/// 1. **Time component** is compared first (NTP64 format)
/// 2. **ID component** breaks ties when times are equal
///
/// This means:
/// - `ts1 < ts2` implies `ts1` **happened before** `ts2` (within causal bounds)
/// - `ts1 == ts2` only if both time and ID are identical (same timestamp instance)
/// - Any two distinct timestamps have a well-defined ordering
///
/// ## Causal Guarantees
///
/// - **Same clock**: If `ts1` was generated before `ts2` on the same HLC,
///   then `ts1 < ts2` (strict causality)
/// - **After remote update**: If clock `A` observes timestamp `ts_B` from clock `B`
///   (via `update()`), any subsequent timestamp from `A` will be greater than `ts_B`
/// - **Concurrent events**: Timestamps from different clocks that have not exchanged
///   updates are ordered deterministically (by time, then ID) but this ordering
///   does not imply causality
///
/// ## Global Uniqueness
///
/// Each [`LogicalClock`] instance has a randomly-generated 128-bit ID, making
/// timestamp collisions statistically impossible across the distributed system.
/// Two timestamps are equal only if they originated from the exact same
/// `new_timestamp()` call.
#[derive(
    Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, serde::Serialize, serde::Deserialize,
)]
