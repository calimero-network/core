...
mod index__private_methods {
    ...
    fn garbage_collect_tombstones() {
        ...
        assert!(<Index<TestStorage>>::get_index(child1_id).unwrap().is_some());
        assert!(<Index<TestStorage>>::get_index(child2_id).unwrap().is_some());
        assert!(<Index<TestStorage>>::get_index(child3_id).unwrap().is_some());
        ...
        // Verify results
        assert!(<Index<TestStorage>>::get_index(child1_id).unwrap().is_none()); // GC'd
        assert!(<Index<TestStorage>>::get_index(child2_id).unwrap().is_some()); // Too recent
        assert!(<Index<TestStorage>>::get_index(child3_id).unwrap().is_some()); // Not deleted
    }
}
