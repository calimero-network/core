...
Entity Conflict → Has CrdtType? → is_builtin_crdt()? → merge_by_crdt_type()
                       ...
                       No                 No (Custom only)
                       ↓                  ↓
                    LWW fallback      WASM callback via __calimero_merge_{TypeName}
```
...
**Custom types**: When `CrdtType::Custom("TypeName")` is encountered, the runtime calls
the WASM export `__calimero_merge_{TypeName}` (generated by `#[app::mergeable]` macro).
...
| `Custom`       | WASM callback         | Calls `__calimero_merge_{TypeName}`   |

*\*These types use "Structured" storage - container metadata only; entries sync separately.*

### Custom Type Merge via WASM

For `CrdtType::Custom("TypeName")`, the storage layer calls the WASM export:

```rust
// Storage layer calls:
merge_by_crdt_type_with_callback(
    &CrdtType::Custom("TeamStats".into()),
    existing_bytes,
    incoming_bytes,
    Some(&wasm_callback),  // RuntimeMergeCallback
)

// Which triggers:
wasm_callback.merge_custom_mut("TeamStats", existing, incoming)
// → Calls WASM export __calimero_merge_TeamStats
```

**To enable custom merges**, add `#[app::mergeable]` to your `Mergeable` impl:
