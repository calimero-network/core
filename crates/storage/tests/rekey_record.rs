...
macro_rules! team_app {
    ($app:ident, $val:ty) => {
        ...
        // `RekeyTarget` is a supertrait of `Mergeable`: re-key the nested
...
// `RekeyTarget` is a supertrait of `Mergeable`. Re-key the root `groups`
...
// ---------------------------------------------------------------------------
// `#[app::state]` macro cascade: prove the MACRO-generated `RekeyTarget` impl +
// `__calimero_register_rekey()` (crates/sdk/macros/src/state.rs) make a nested
// custom-struct value CONVERGE at runtime — not merely compile. Mirrors
// `FixedApp`, but the `Mergeable`, `RekeyTarget`, and cascade registration are
// all generated by `#[app::state]` instead of hand-written.

#[derive(BorshSerialize, BorshDeserialize, Default, Mergeable)]
#[borsh(crate = "calimero_sdk::borsh")]
struct AppStateStats {
    wins: Counter,
}
...
#[derive(Default)]
struct AppStateTeam {
    teams: UnorderedMap<String, AppStateStats>,
}

// `#[app::state]` requires `AppState: AppStateInit` (normally supplied by an
// `#[app::init]` under `#[app::logic]`). This test drives the state directly via
// the `drive()` harness, not the WASM logic exports, so we satisfy the bound by
// hand instead of pulling in the full `#[app::logic]` machinery.
impl calimero_sdk::state::AppStateInit for AppStateTeam {
    type Return = AppStateTeam;
}

impl TeamApp for AppStateTeam {
    fn record_win(&mut self, team: &str) -> app::Result<()> {
        let mut stats = self.teams.entry(team.to_owned())?.or_default()?;
        stats.wins.increment()?;
        Ok(())
...
#[test]
#[serial]
fn app_state_macro_generated_cascade_makes_nested_value_converge() {
    env::reset_environment();
    register_crdt_merge_for_test::<AppStateTeam>();
    // Register via the MACRO-GENERATED hook (not a manual
    // `register_rekey_if_supported!`): this is what proves `#[app::state]` wired
    // the `RekeyTarget` cascade so the nested `AppStateStats` value gets a
    // deterministic id and its counter converges instead of being LWW'd.
    AppStateTeam::__calimero_register_rekey();
