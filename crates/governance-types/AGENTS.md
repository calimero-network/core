...
| `wire::SignedAck`, `wire::SignedReadinessBeacon`, `wire::SignedMigrationHeartbeat`, `wire::ReadinessProbe` | struct | Signed gossip primitives layered on top of ops (see Mental Model); `SignedReadinessBeacon` additionally carries an `admission_proof: Option<SignedGroupOpenInvitation>` field, positioned immediately before `signature` and covered by the beacon signature |
...
- **`SignedReadinessBeacon`'s `admission_proof` field did NOT follow the `SignedMigrationHeartbeat` precedent above, and that gap is unresolved, not overlooked.** The field is a plain derived-borsh struct member, signed as part of the body, not a hand-rolled trailing read - so unlike the heartbeat, this crate's usual mixed-fleet tolerance was not applied here. `NamespaceTopicMsg`/`GroupTopicMsg` decode via `borsh::from_slice`, which rejects trailing bytes, so an un-upgraded node fails closed on any new-format beacon. Because the struct's shape changed for every beacon, not only proof-carrying ones, this drops EVERY beacon an upgraded node emits, `admission_proof: None` included. Beacon-driven liveness and divergence detection go dark between a mismatched pair until the rollout completes, then self-heal; nothing is persisted and no state is corrupted, but the change requires a coordinated cluster upgrade.
