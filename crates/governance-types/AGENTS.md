...
| `KeyEnvelope` | struct | ECDH-wrapped group key for one recipient: `recipient`, `sender`, `nonce`, `ciphertext`, `signature` |
| `EnvelopeRecipient` | enum (NOT `#[non_exhaustive]`) | Who a `KeyEnvelope` is for **and** the ECDH key that opens it: `Member { identity, ephemeral_pk }` (Ed25519 identity agreement - the bootstrap form) or `Device { device, ephemeral_pk }` (native X25519 to a certified KEM key). Tag 0 / tag 1, permanently |
| `KeyRotation` | struct | `{ new_key_id, envelopes: Vec<KeyEnvelope> }` attached to `MemberRemoved`; one envelope per remaining recipient, none for the removed member. A single bundle may mix both addressing modes |
...
- **`SIGNED_GROUP_OP_SCHEMA_VERSION` (currently 14) and `SIGNED_NAMESPACE_OP_SCHEMA_VERSION` (currently 7) are checked strictly-equal on `verify_signature()`.** Bumping either means every op's content hash changes and old-shape ops are rejected outright, not migrated - so every node and devnet re-bootstraps. **While the project is alpha that is a normal cost, not an event to schedule around.** Bump when the layout should change, and prefer the correct shape over a rollout-preserving workaround. Read the version-history doc comments above each constant first, for the history rather than for permission: some past changes (e.g. `GroupKeyRotated`, the v6/v7 cascade variants) deliberately did NOT bump, to avoid making older peers reject every op rather than just the new variant. That trade was about protecting a live fleet; it is not a reason to avoid a bump now, and treating it as one is how a straightforward layout change turns into a permanent compatibility shim. There is no backwards-compatibility requirement in alpha.
- **A root-signed statement is not a fresh op.** Every member of a group decrypts the ops it carries, so any of them can re-wrap a past `AccountDeviceDescoped`'s scope in a new op with their own signature and nonce - past the signer gate and past the nonce window. The binding's stamped `scope_epoch` is what refuses it: a descope applies only where the binding was made under a strictly older scope. Any future op whose authority is a carried statement needs the same kind of ordering, because re-signing is free.
- **Removing a variant is a flag day, and the bump must ride the same commit.** Deleting a `GroupOp` variant renumbers every later ordinal, so a straggler op signed against the old layout would decode as the wrong variant. Bumping `SIGNED_GROUP_OP_SCHEMA_VERSION` in the same commit makes it hit the version gate before decode instead. v10 removed three variants at once for exactly this reason - each extra deletion is free inside one flag day and costs another one afterwards.
...
- **`SignedReadinessBeacon` carries no optional fields, and a new one must follow the `SignedMigrationHeartbeat` precedent above.** The beacon previously carried an `admission_proof: Option<SignedGroupOpenInvitation>` as a plain derived-borsh member signed as part of the body - not a hand-rolled trailing read - so this crate's usual mixed-fleet tolerance never applied to it. That field is gone with the direct-admission cutover, which restores the pre-proof layout exactly. Both the addition and the removal changed the struct's shape for EVERY beacon, not only proof-carrying ones: `NamespaceTopicMsg`/`GroupTopicMsg` decode via `borsh::from_slice`, which rejects trailing bytes, so a mismatched node fails closed and drops every beacon its peer emits. Beacon-driven liveness and divergence detection go dark between a mismatched pair until the rollout completes, then self-heal; nothing is persisted and no state is corrupted, but either direction requires a coordinated cluster upgrade. `proof_carrying_layout_rejects_new_encoding` in `wire.rs` pins the fail-closed half of that contract.
