...
/// Node identity configuration containing the libp2p network identity.
///
/// Serialized as the `[identity]` TOML section with `peer_id` and `keypair`.
/// Group/namespace identities are now stored per-namespace in the datastore.
#[derive(Debug)]
...
pub struct SyncConfig {
    ...
    /// Per-session deadline for one sync session run on the node's
    /// `SyncSessionActor` (calimero-network/core #2319). Optional in the
    /// config file; defaults to 15 s (shorter than `timeout_ms`, which
    /// stays an internal per-step budget).
    #[serde(
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> de::Visitor<'de> for IdentityVisitor {
            ...
            fn expecting(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
                formatter.write_str("an identity configuration with peer_id and keypair")
            }
