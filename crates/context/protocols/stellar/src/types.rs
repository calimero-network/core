...
use calimero_context_config_core::repr::{Repr, ReprBytes, ReprError, ReprTransmute};
use calimero_context_config_core::types::{Application, ApplicationId, ApplicationMetadata, ApplicationSource, BlobId, Capability};
use calimero_context_config_core::{ContextRequest, ContextRequestKind, RequestKind};
...
impl<'a> From<StellarApplication> for Application<'a> {
    fn from(value: StellarApplication) -> Self {
        ...
        Application::new(
            Repr::new(ApplicationId::from_bytes(|bytes| {
                bytes.copy_from_slice(&StellarBytes32(value.id).as_bytes());
                Ok(32)
            }).expect("infallible conversion")),
            Repr::new(BlobId::from_bytes(|bytes| {
                bytes.copy_from_slice(&StellarBytes32(value.blob).as_bytes());
                Ok(32)
            }).expect("infallible conversion")),
            value.size,
...
// Stellar-specific wrapper for BytesN<32>
pub struct StellarBytes32(pub BytesN<32>);

impl ReprBytes for StellarBytes32 {
    type EncodeBytes<'a>
        = [u8; 32]
    where
        Self: 'a;
        ...
        F: FnOnce(&mut Self::DecodeBytes) -> Result<usize, bs58::decode::Error>,
    {
        ...
        match f(&mut bytes) {
            Ok(_) => {
                let env = Env::default();
                Ok(StellarBytes32(BytesN::from_array(&env, &bytes)))
            }
