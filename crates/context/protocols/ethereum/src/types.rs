    ...
    fn from(sol_app: SolApplication) -> Self {
        Self::new(
            Repr::new(ApplicationId::from_bytes(|bytes| {
                bytes.copy_from_slice(sol_app.id.as_slice());
                Ok(32)
            }).expect("infallible conversion")),
            Repr::new(BlobId::from_bytes(|bytes| {
                bytes.copy_from_slice(sol_app.blob.as_slice());
                Ok(32)
            }).expect("infallible conversion")),
            sol_app.size,
            ApplicationSource(std::borrow::Cow::Owned(sol_app.source)),
            ApplicationMetadata(Repr::new(std::borrow::Cow::Owned(
