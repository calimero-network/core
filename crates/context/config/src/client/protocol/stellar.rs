...
#[serde(try_from = "serde_creds::Credentials", into = "serde_creds::Credentials")]
...
mod serde_creds {
    ...
    impl From<super::Credentials> for Credentials {
        fn from(creds: super::Credentials) -> Self {
            // For Stellar, account_id is represented exactly the same as public_key, so we skip it
            Self {
                account_id: None,
                public_key: creds.public_key,
                secret_key: creds.secret_key,
