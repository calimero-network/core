...
use soroban_client::soroban_rpc::{
    SimulateTransactionResponse, SendTransactionStatus, TransactionStatus,
};
...
use soroban_client::xdr::{ScVal, ReadXdr, WriteXdr, Limits};
...
impl Network {
    async fn query(
        ...
        if !args.is_empty() {
            ...
            let sc_val: ScVal = ScVal::from_xdr_base64(&base64_args, Limits::none())
        ...
        let result: Result<SimulateTransactionResponse, Error> = self
            .client
        ...
        match response.to_result() {
            Some((sc_val, _auth)) => {
                ...
                let xdr_bytes = sc_val.to_xdr_base64(Limits::none())
                    .map_err(|_| StellarError::Custom {
                        operation: ErrorOperation::Query,
                                ...
                                Ok(response) => {
                                    match response.status {
                                        TransactionStatus::Success => {
                                            if let Some((_meta, return_value)) = response.to_result_meta() {
        ...
        match result.flatten() {
            Some(sc_val) => match sc_val {
                ...
                | ScVal::ContractInstance(_)) => {
                    let xdr_bytes = val.to_xdr_base64(Limits::none())
                        .map_err(|_| StellarError::Custom {
                            operation: ErrorOperation::Mutate,
