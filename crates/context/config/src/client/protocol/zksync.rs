...
impl std::fmt::Display for CredentialsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CredentialsError::InvalidAccountId => write!(f, "Invalid account ID"),
            CredentialsError::InvalidSecretKey => write!(f, "Invalid secret key"),
        }
    }
}
...
impl Network {
    ...
    pub async fn mutate(
        ...
        let block_number = receipt.block_number().ok_or_else(|| ZkSyncError::Custom {
            operation: ErrorOperation::Mutate,
