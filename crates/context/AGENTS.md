...
| `account_follow` | `AccountNamespaceGained`, `AccountNamespaceLeft`, `AccountDeviceCertified` (this node's own device) | Follows a namespace the account gained whose application this device's registry scope covers, walks the whole set when this device's own scope arrives, and pulls then unsubscribes from one the account left. Every start sweeps both ways: it follows what the set covers, and unfollows a namespace the set no longer names whose member row also shows the account gone |
