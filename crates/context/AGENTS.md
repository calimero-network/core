...
| `account_follow` | `AccountNamespaceGained`, `AccountNamespaceLeft`, `AccountDeviceCertified`, `DeviceRevoked` (with a proof) | Follows a namespace the account gained whose application this device's registry scope covers, walks the whole set when this device's own scope arrives, and pulls then unsubscribes from one the account left. Every start sweeps both ways: it follows what the set covers, and unfollows a namespace this node has folded whose set row is gone and whose member row shows the account gone - all three, so an unsynced namespace is never mistaken for a left one. A sibling this account certifies is projected into the namespaces here its scope covers, and a revocation the account proved is carried into the ones the revoker could not reach - never the account namespace itself, which is where both arrive |
