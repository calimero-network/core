...
| `enrol_listener` | periodic sweep (+ `GroupKeyDelivered` for latency) | Enrols this node's account once the namespace scope key is available, so `account create` is never something an operator has to remember. The **sweep** is the guarantee, not the event: `GroupKeyDelivered` fires only for an applied `RootOp::KeyDelivery`, while a joiner normally gets its key from the join response / `recover_missing_group_keys` and a creator mints its own — neither emits anything. Needs no persisted worklist: "holds a key, holds no device" is derivable from stored state |
...
| `src/auto_follow.rs`, `src/self_purge.rs`, `src/tee_subgroup_admit.rs`, `src/rotation_listener.rs`, `src/enrol_listener.rs` | The five background listeners spawned in `Actor::started` |
...
- **`Actor::started` spawn ordering is load-bearing** - see the comment block above `auto_follow::spawn`; don't reorder the listener spawns without re-reading it. `enrol_listener` is order-independent relative to the others (it subscribes synchronously before spawning, like they do), but the same shutdown-then-spawn rebinding applies.
- **Enrolment is implicit, and it cannot happen at join time.** `AccountDeviceLinked` is an *encrypted* `GroupOp`, so it needs the namespace scope key — which a joiner does not have at join time. Enrolling on join deadlocks on the very link that would get it the key, which is why `create_account` refuses when the keyring is empty.
- **There is no single event meaning "this node now has a group key", so do not build one in.** `OpEvent::GroupKeyDelivered` is emitted only from `apply_received_group_key_envelope` (an applied `RootOp::KeyDelivery`). A joiner normally acquires its key through the direct join response and `recover_missing_group_keys`; a namespace creator mints its own. Neither emits an event — measured, not assumed: a three-node scenario that demonstrably had keys logged zero deliveries. `enrol_listener` therefore treats the event as a latency optimisation and relies on a periodic sweep for correctness.
