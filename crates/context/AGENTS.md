...
| `enrol_listener` | `GroupKeyDelivered` + startup sweep | Enrols this node's account once the namespace scope key arrives, so `account create` is never something an operator has to remember. Needs no persisted worklist: "holds a key, holds no device" is derivable from stored state |
...
- **Enrolment is implicit, and the trigger is key delivery, not the join.** `AccountDeviceLinked` is an *encrypted* `GroupOp`, so it needs the namespace scope key — which a joiner does not have at join time. Enrolling on join deadlocks on the very link that would get it the key, which is why `create_account` refuses when the keyring is empty and why `enrol_listener` waits for `GroupKeyDelivered`.
