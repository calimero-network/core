    ...
    fn get_or_fetch_context(
        ...
        if was_cached {
            ...
            if let Some(meta) = handle.get(&key)? {
                ...
                // Refresh application_id too. An upgrade to this context (or
                ...
                // path's post-upgrade re-fetch) resolve the WASM module
                // from `application_id`; if the cache still holds the
                // pre-upgrade id, the method runs the OLD module against the
                // freshly-migrated (new-shaped) state — a borsh "Not all
                // bytes read" panic. Keeping app_id in lockstep with the DB
                // closes that window.
