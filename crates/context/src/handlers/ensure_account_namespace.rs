            ...
            async move {
                    ...
                    .await
                {
                    Ok(_created) => {
                        ...
                        let credential =
                            crate::join_credential::build(&datastore, &namespace_id, &signer_pk)?;
                        ...
                        .await
                        {
                            Ok(report) => {
                                report
                                    .observe("ensure_account_namespace", "AccountDeviceCertified");
                            }
                            Err(err) => warn!(
                                ?err,
                                ?namespace_id,
                                "created the account namespace, but could not record this \
                                 device in it"
                            ),
                        }
                    }
