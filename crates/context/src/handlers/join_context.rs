...
async fn join_context(
    ...
    debug!("🔍 Join context: context_id={}, invitee_id={}, protocol={}, network_id={}, contract_id={}", 
    ...
    if context_client
        ...
        .is_some()
    {
        debug!("ℹ️  Already joined context: context_id={}, invitee_id={}", context_id, invitee_id);
        return Ok((context_id, invitee_id));
    }

    debug!("🔑 Looking up stored identity for invitee_id={}", invitee_id);
    ...
    debug!("✅ Identity validation passed for invitee_id={}", invitee_id);

    let mut config = None;

    let has_context = context_client.has_context(&context_id)?;
    debug!("🏗️  Context exists check: context_id={}, has_context={}", context_id, has_context);

    if !has_context {
        debug!("🆕 Creating new context config for context_id={}", context_id);
        ...
        debug!("📋 Using existing context config for context_id={}", context_id);
    }

    debug!("🔄 Starting context config sync for context_id={}", context_id);
    let _ignored = context_client
        .sync_context_config(context_id, config)
        .await?;
    debug!("✅ Context config sync completed for context_id={}", context_id);

    let is_member = context_client.has_member(&context_id, &invitee_id)?;
    debug!("👥 Member check: context_id={}, invitee_id={}, is_member={}", context_id, invitee_id, is_member);
    
    if !is_member {
        warn!("❌ Failed to join context: invitee_id={} is not a member of context_id={}", invitee_id, context_id);
    ...
    debug!("💾 Updating identity for context_id={}, invitee_id={}", context_id, invitee_id);
    ...
    debug!("🗑️  Deleting temporary identity for invitee_id={}", invitee_id);
    ...
    info!("🎉 Successfully joined context: context_id={}, invitee_id={}", context_id, invitee_id);
