...
mod tests {
    ...
    /// A second device of this node's own account, certified by its root exactly
    /// as `pair_device_complete` would.
    fn paired_device(store: &Store, seed: u8, applications: Vec<[u8; 32]>) -> DeviceId {
        let devices = NodeDeviceRepository::new(store);
        let root = devices.ensure_account_root().expect("root");
        ...
        devices
            .remember_device_cert(
                &proof,
                &applications.into_iter().map(app).collect::<Vec<_>>(),
            )
    ...
    fn a_node_that_does_not_hold_the_account_is_refused() {
        ...
        let device = paired_device(&store, 0x31, vec![]);
    ...
    fn a_revoked_device_is_refused_rather_than_repaired() {
        let store = a_node_holding_its_own_account();
        let device = paired_device(&store, 0x32, vec![]);
    ...
    fn naming_applications_extends_the_stored_scope_persistently() {
        let store = a_node_holding_its_own_account();
        let device = paired_device(&store, 0x33, vec![APP_ONE]);
    ...
    fn re_naming_an_application_the_device_already_covers_changes_nothing() {
        let store = a_node_holding_its_own_account();
        let device = paired_device(&store, 0x34, vec![APP_ONE]);
