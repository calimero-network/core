...
use crate::group::{
    ...
    DetachContextFromGroupRequest, EnsureAccountNamespaceRequest, FollowNamespaceRequest,
    ...
    ListNamespacesForApplicationRequest, ListNamespacesRequest, MigrationStatus,
    NamespaceParticipation, NamespaceSummary, PairDeviceCompleteRequest, PairDeviceInitRequest,
    RelinkDeviceRequest, RemoveGroupMembersRequest, ResyncContextRequest, ResyncContextResponse,
...
impl ContextClient {
    ...
    pub async fn acquire_lock(&self, context_id: &ContextId) -> Option<ContextAtomicKey> {
        ...
        if let Err(err) = self
            ...
            .await
        {
            tracing::error!(%err, "context manager mailbox closed while acquiring context lock");
            return None;
        }
        ...
        match receiver.await {
            Ok(key) => key,
            Err(err) => {
                tracing::error!(%err, "context manager dropped the lock response channel");
                None
            }
        }
    }
    ...
    forward_to_actor!(
        ...
        eyre::Result<ResyncContextResponse>
    );
