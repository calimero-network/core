...
- **Old manifests must keep deserializing.** Every field added after `wasm-abi/1` shipped (`intent`, `xcall_callable`, `xcall_callers`, `state_version`, `migrations`, `doc`, `returns_doc`, `destructive`, `idempotent`) is `#[serde(default, skip_serializing_if = ...)]` with a documented fail-safe default (`Unspecified` -> write lock, `AnyInNamespace` -> the historical open policy, `state_version` -> `1` via `state_version_or_default()`, `doc`/`returns_doc` -> absent means undocumented, `destructive`/`idempotent` -> absent means false). Adding a field without a safe default and without the skip predicate breaks every already-compiled app's manifest.
