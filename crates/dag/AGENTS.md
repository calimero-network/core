...
| Item | Kind | Purpose |
| --- | --- | --- |
| `CausalDelta<T>` | struct | `id`, `parents: Vec<[u8; 32]>`, generic `payload: T`, `hlc: HybridTimestamp`, `kind: DeltaKind`. No root hash: a per-delta `expected_root_hash` was removed (#3557) — it was sender-asserted, outside the `compute_id` preimage, and the node has always stored the hash it COMPUTED |
| `CausalDelta::new(id, parents, payload, hlc)` | fn | Regular-kind constructor |
| `CausalDelta::checkpoint(id, root_hash)` | fn | Snapshot-boundary delta: genesis parent, `T::default()` payload, `DeltaKind::Checkpoint { root_hash }`. The root hash is derived LOCALLY from a snapshot this node took |
| `CausalDelta::checkpoint_root_hash()` | fn | `Some(root_hash)` for a checkpoint, `None` otherwise |
| `CausalDelta::into_genesis()` | fn | Marks the delta `DeltaKind::Genesis`, which is what licenses an empty `parents` list — see the invariants below |
| `CausalDelta::is_genesis()` | fn | `kind == Genesis` |
...
| `MAX_REFUSED_TRACKED` | const | `100_000` - cap on the refused-delta tracking set |
...
| `refuse_delta(id, applier) -> Vec<[u8; 32]>` | Records `id` as refused (a satisfied parent, never requested) and applies the pending deltas it released; a delta already applied is left alone. `is_refused(id)` reads the set |
...
`add_delta` is the single entry point. `can_apply` decides readiness: an EMPTY `parents` list is accepted only from a `DeltaKind::Genesis` delta (and rejected with a `warn!` otherwise); for a non-empty list, every parent must be either the zero-hash genesis, a deliberately-pruned ancestor (see below), a refused delta (see below), or both `applied` and still present in `deltas`. If ready, `apply_delta` calls into the injected `DeltaApplier<T>::apply`, marks the delta applied, and updates `heads` (parents drop out, the new delta becomes a head - so concurrent deltas from the same parent produce multiple heads until a later delta lists both as parents and merges them back to one). If not ready, the delta is stored in `pending` and indexed by `pending_children` (parent id -> waiting child ids).
...
- **An empty `parents` list is valid ONLY for `DeltaKind::Genesis`** (#3126). `can_apply`'s `all()` over an empty iterator is vacuously true, which used to make every parentless delta apply instantly as a disconnected head — including one whose parents a bug dropped or a peer stripped. Anything parentless that does not declare `Genesis` is now rejected, loudly: a rejected delta PENDS rather than failing, and pends forever since nothing will ever satisfy a parent it never named, so the `warn!` is what distinguishes it from a stalled sync.
- **Do not narrow who may be marked `Genesis` based on op variant.** `read_head_record` returns an EMPTY `parent_hashes` whenever no head is persisted, so on the governance plane "no parents" means "signed against an empty local head" — routine for any not-yet-caught-up node, with any op variant — not "genesis". Narrowing this to `NamespaceCreated` strands those ops and governance never converges. It has been tried twice (#3116, and again during #3565); the `group-join-mesh-not-ready` e2e is what catches it.
...
- **Refused parents count as satisfied too, for a different reason.** `refuse_delta` is for a caller whose authorization refused a delta outright (the node refuses a revoked device's deltas, core#4070). The id goes into `refused` (bounded FIFO via `refused_order`, capped at `MAX_REFUSED_TRACKED`): `can_apply` treats it as a satisfied parent, and `get_missing_parents` and `pending_stats` skip it. Its pending children cascade at once. It does NOT bar the id from a later `add_delta`: a delta's id content-addresses parents, actions and HLC but not the author, so someone holding a revoked key can re-sign an honest delta's content, and the honest copy must still land when the caller accepts it. Without this, an honest delta built on a refused one pends until `cleanup_stale` drops it, and every sync until then asks peers for a parent the node will only refuse again. The refused delta never applies, so nothing retires ITS parents from `heads`; the next local delta cites them as well, which is harmless. The set is in memory only: after a restart a child naming the refused id asks for it, the caller refuses it again, and it is re-recorded.
