...
| `RootKeyHandoff::sign(sk, account, from_epoch, new_pk)` | fn | Mint one. `new_pk` is a `RootPublicKey`, so an account born Ed25519 can hand off to a P-256 root |
| `RootPublicKey` | enum | A root key tagged with its algorithm: `Ed25519([u8; 32])` (variant 0, permanently) or `P256([u8; 33])` (SEC1 compressed, variant 1). A genesis is always Ed25519; a handoff may name either |
| `RootPublicKey::verify(payload, signature)` | fn | Verify a 64-byte signature over a 32-byte digest under this key's own algorithm. P-256 uses `verify_prehash` over raw `r ‖ s`, because the payload already is the digest |
| `RootPublicKey::to_wire()` | fn | Fixed 34-byte `tag ‖ key ‖ pad`, for hashing and map keys; the tag comes first so two algorithms can never collide |
| `RootKeyError` | enum | Why a root key could not verify: `MalformedKey` or `SignatureInvalid` |
...
| `src/root_pk.rs` | `RootPublicKey` + `verify` / `to_wire`, `RootKeyError` — the one place a root key's algorithm is decided |
...
- **`root_key_at_epoch` caps the chain at `MAX_ROOT_KEY_HANDOFFS` (1024) before reading anything.** Each entry costs a signature verification (Ed25519 or P-256, by the outgoing key's tag) and the function is reachable from untrusted bytes, so an uncapped chain is verification amplification. The cap lives here as well as at the governance wire boundary because `calimero-op` has no bounds layer at all - relying on every caller to bound the field first would leave the unified plane open.
...
- **Only the root is algorithm-tagged; a device key is always Ed25519.** A root held in a secure element or a PIV slot is P-256, and `RootPublicKey` carries that. Everything a device signs still verifies as Ed25519, so a hardware root changes who can certify a device, never how that device's ops verify. A genesis stays Ed25519 too, because it is the preimage of the `AccountId`: tagging it would change every account id.
