...
### Mergeable Macro (Custom Type Merge)

Generates WASM export for custom type merging during sync:
...
// Generates __calimero_merge_TeamStats WASM export
#[app::mergeable]
...
- Generates `__calimero_merge_{TypeName}` WASM export automatically
- Called by runtime when entities with `CrdtType::Custom("TypeName")` conflict
- Use for complex nested structures or app-specific merge rules
- See `apps/team-metrics-custom` for full example

**WASM Exports Generated by SDK Macros:**

| Macro | Export | Purpose |
|-------|--------|---------|
| `#[app::state]` | `__calimero_merge_root_state` | Root state conflicts |
| `#[app::state]` | `__calimero_alloc` | Memory allocation |
| `#[app::state]` | `__calimero_register_merge` | Register merge function |
| `#[app::mergeable]` | `__calimero_merge_{TypeName}` | Custom entity merges |
