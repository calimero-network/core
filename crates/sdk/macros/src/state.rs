...
fn generate_mergeable_impl(
    ...
    quote! {
        // Auto-generated by #[app::state]. Calls Mergeable::merge on every field
...
fn generate_test_state_impl(
    ...
    quote! {
        ...
        impl #impl_generics ::calimero_sdk::testing::TestState for #ident #ty_generics #where_clause {
            ...
            fn __test_with_device(id: [u8; 32], f: &mut dyn ::core::ops::FnMut()) {
                ...
                for __i in 0..__len {
                    ...
                    if __owned && __stale {
                        ...
                        if let ::core::option::Option::Some(__v) = __val {
                            ...
                            // Owned + stale but unreadable (read error / absent
                            // slot): genuinely unconverted. Count as remaining so
                            // this tally matches `count_my_pending` (which counts
                            // every owned+stale slot), else `authored_remaining`
                            // never reaches zero while migrate reports complete.
                            __remaining = __remaining.saturating_add(1);
                        }
                    }
                }
